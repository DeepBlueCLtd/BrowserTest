var SonarQuiz=function(e){"use strict";function t(e){if(e.length<2)return"**";if(2===e.length)return e;return e.slice(0,2)+"*".repeat(e.length-2)}function n(e){if(null===e||"object"!=typeof e)return e;const s={};for(const[o,r]of Object.entries(e))"name"!==o&&"passwordHash"!==o&&(s[o]="serviceId"!==o||"string"!=typeof r?"object"!=typeof r||null===r?r:n(r):t(r));return s}function s(e,t){void 0!==t?console.log(`[INFO] ${e}`,n(t)):console.log(`[INFO] ${e}`)}function o(e,t){if(t instanceof Error){const n={name:t.name,message:t.message};console.error(`[ERROR] ${e}`,n)}else void 0!==t?console.error(`[ERROR] ${e}`,n(t)):console.error(`[ERROR] ${e}`)}function r(e,t){void 0!==t?console.warn(`[WARN] ${e}`,n(t)):console.warn(`[WARN] ${e}`)}function a(e){const t=[],n=[];if(!e.classList.contains("qd-quiz"))return t.push('Table must have class "qd-quiz"'),{element:e,questions:n,errors:t};const s=Array.from(e.querySelectorAll("tbody tr"));return 0===s.length?(t.push("Quiz table has no data rows"),{element:e,questions:n,errors:t}):(s.forEach((e,s)=>{const o=Array.from(e.querySelectorAll("td"));if(3!==o.length)return void t.push(`Row ${s+1} has ${o.length} columns, expected 3 (Question | Answer | Detail)`);const r=o[0],a=o[1],d=o[2];if(!r||!a||!d)return;const c=r.textContent?.trim()||"";if(!c)return void t.push(`Row ${s+1} has empty question text`);const l=a.textContent?.trim()||"";if(!l)return void t.push(`Row ${s+1} has empty answer`);const u=d.querySelector("ol");if(u){const e=(h=u,Array.from(h.querySelectorAll("li")).map(e=>e.textContent?.trim()||"").filter(e=>e.length>0));if(0===e.length)return void t.push(`Row ${s+1} MCQ has no options in <ol>`);n.push({text:c,kind:"mcq",correctAnswer:l,options:e})}else{const e=d.textContent?.trim()||"",o=parseFloat(e);if(isNaN(o))return void t.push(`Row ${s+1} appears to be numeric but has invalid tolerance: "${e}"`);n.push({text:c,kind:"numeric",correctAnswer:l,tolerance:o})}var h}),{element:e,questions:n,errors:t.length>0?t:void 0})}function d(e,t){if(!t||""===t.trim())return!1;const n=t.trim();if("mcq"===e.kind)return n===e.correctAnswer;{const t=parseFloat(n),s=parseFloat(e.correctAnswer);if(isNaN(t)||isNaN(s))return!1;const o=e.tolerance??0;return Math.abs(t-s)<=o}}const c=18e5,l={SESSION:"qd/session",CACHE:"qd/state",INSTRUCTOR:"qd/instructor"};class SessionService{createSession(e,t,n){const o=new Date,r=o.toISOString(),a={serviceId:e,name:t,release:n,loginTime:r,lastActivity:r,expiresAt:new Date(o.getTime()+c).toISOString(),instructorUnlocked:!1};return this.saveSession(a),s(`Session created for ${e} (${t})`),this.emitEvent("qd:login",{serviceId:e,name:t,release:n,loginTime:r}),a}getSession(){try{const e=sessionStorage.getItem(l.SESSION);if(!e)return null;const t=JSON.parse(e);return t.serviceId&&t.release&&t.expiresAt?t:(r("Invalid session data, missing required fields"),null)}catch(e){return o("Failed to parse session data",e),null}}updateActivity(){const e=this.getSession();if(!e)return;const t=new Date;e.lastActivity=t.toISOString(),e.expiresAt=new Date(t.getTime()+c).toISOString(),this.saveSession(e)}isExpired(){const e=this.getSession();if(!e)return!0;return new Date>=new Date(e.expiresAt)}clearSession(){const e=this.getSession();sessionStorage.removeItem(l.SESSION),sessionStorage.removeItem(l.CACHE),sessionStorage.removeItem(l.INSTRUCTOR),sessionStorage.removeItem("qd/instructor/showAnswers"),e&&(s(`Session cleared for ${e.serviceId}`),this.emitEvent("qd:logout",{serviceId:e.serviceId,timestamp:(new Date).toISOString()}))}unlockInstructor(){const e=this.getSession();e&&(e.instructorUnlocked=!0,e.unlockTime=(new Date).toISOString(),this.saveSession(e),s("Instructor mode unlocked"),this.emitEvent("qd:instructor-unlock",{timestamp:e.unlockTime}))}lockInstructor(){const e=this.getSession();e&&(e.instructorUnlocked=!1,delete e.unlockTime,this.saveSession(e),s("Instructor mode locked"),this.emitEvent("qd:instructor-lock",{timestamp:(new Date).toISOString()}))}isInstructorUnlocked(){const e=this.getSession();return!0===e?.instructorUnlocked}getCache(){try{const e=sessionStorage.getItem(l.CACHE);return e?JSON.parse(e):null}catch(e){return o("Failed to parse cache data",e),null}}saveCache(e){try{sessionStorage.setItem(l.CACHE,JSON.stringify(e))}catch(t){o("Failed to save cache",t)}}clearCache(){sessionStorage.removeItem(l.CACHE)}saveSession(e){try{sessionStorage.setItem(l.SESSION,JSON.stringify(e))}catch(t){o("Failed to save session",t)}}emitEvent(e,t){try{const n=new CustomEvent(e,{detail:t,bubbles:!0});document.dispatchEvent(n)}catch(n){o(`Failed to emit event ${e}`,n)}}}function u(e,t){const n=t.answers.length,s=t.answers.filter(e=>""!==e.answer.trim()).length,o=t.answers.filter(e=>e.success).length;return{state:t.state,total:n,answered:s,correct:o,last:t.lastAttempted,answers:t.answers,analysis:t.analysis}}class Debouncer{constructor(){this.timers=new Map}debounce(e,t,n=200){const s=this.timers.get(e);void 0!==s&&clearTimeout(s);const o=setTimeout(()=>{this.timers.delete(e),t()},n);this.timers.set(e,o)}cancel(e){const t=this.timers.get(e);return void 0!==t&&(clearTimeout(t),this.timers.delete(e),!0)}cancelAll(){let e=0;for(const t of this.timers.values())clearTimeout(t),e++;return this.timers.clear(),e}isPending(e){return this.timers.has(e)}getPendingCount(){return this.timers.size}}function h(e){const t=e.querySelector("tbody");return t?Array.from(t.querySelectorAll("tr")):[]}function p(e){return Array.from(e.cells)}function g(e){return e&&e.textContent?.trim()||""}function m(e,t,n){return document.createElement(e)}function f(e,...t){e.classList.add(...t)}function b(e,...t){e.classList.remove(...t)}function v(e,t,n){const s=new CustomEvent(e,{detail:t,bubbles:!0,composed:!0,cancelable:!1});return document.dispatchEvent(s)}function y(e,t,n,s){const o=new CustomEvent(t,{detail:n,bubbles:!0,composed:!0,cancelable:!1});return e.dispatchEvent(o)}function w(e){try{const t=sessionStorage.getItem(e);return t?JSON.parse(t):null}catch(t){return r(`Failed to parse JSON from sessionStorage key: ${e}`,t),null}}function x(e,t){try{const n=JSON.stringify(t);return sessionStorage.setItem(e,n),!0}catch(n){return r(`Failed to store JSON in sessionStorage key: ${e}`,n),!1}}function S(){const e=[];for(let t=0;t<sessionStorage.length;t++){const n=sessionStorage.key(t);n&&n.startsWith("qd/")&&e.push(n)}for(const t of e)sessionStorage.removeItem(t);return e.length}function E(e,t){return`qd/${e}/u${t}`}class StorageError extends Error{constructor(e,t,n){super(e),this.operation=t,this.cause=n,this.name="StorageError",n?o(`Storage error in ${t}: ${e}`,n):o(`Storage error in ${t}: ${e}`)}}class StorageNotInitializedError extends StorageError{constructor(e){super("Storage adapter not initialized. Call init() first.",e),this.name="StorageNotInitializedError"}}class StorageQuotaError extends StorageError{constructor(e){super("Storage quota exceeded. Please clear old data or free up space.",e),this.name="StorageQuotaError"}}const $="BrowserTest",C="students",A="backups";class IndexedDBStorageAdapter{constructor(e=$){this.db=null,this.initPromise=null,this.dbName=e}async init(){return this.initPromise?this.initPromise:this.db?Promise.resolve():(this.initPromise=new Promise((e,t)=>{const n=indexedDB.open(this.dbName,2);n.onerror=()=>{this.initPromise=null,t(new StorageError("Failed to open database","init",n.error))},n.onsuccess=()=>{if(this.db=n.result,s(`IndexedDB opened: ${this.dbName} v${this.db.version}, stores: [${Array.from(this.db.objectStoreNames).join(", ")}]`),!this.db.objectStoreNames.contains(C)||!this.db.objectStoreNames.contains(A)){r(`Database corrupted (missing stores). Found: [${Array.from(this.db.objectStoreNames).join(", ")}]`),this.db.close(),this.db=null;const n=indexedDB.deleteDatabase(this.dbName);return n.onsuccess=()=>{r("Corrupted database deleted, retrying init..."),this.initPromise=null,this.init().then(e).catch(t)},void(n.onerror=()=>{this.initPromise=null,t(new StorageError("Failed to delete corrupted database","init",n.error))})}this.initPromise=null,e()},n.onupgradeneeded=e=>{const t=e,n=e.target.result;s(`IndexedDB upgrade: ${this.dbName} v${t.oldVersion} → v${t.newVersion}`);try{if(n.objectStoreNames.contains(C))s("Students store already exists, skipping");else{s("Creating students object store...");const e=n.createObjectStore(C,{keyPath:null});e.createIndex("by-release","release",{unique:!1}),e.createIndex("by-service-id","serviceId",{unique:!1}),s("Students store created with indexes")}if(n.objectStoreNames.contains(A))s("Backups store already exists, skipping");else{s("Creating backups object store...");const e=n.createObjectStore(A,{keyPath:null});e.createIndex("by-original-key","originalKey",{unique:!1}),e.createIndex("by-timestamp","timestamp",{unique:!1}),s("Backups store created with indexes")}s(`Upgrade complete. Stores: [${Array.from(n.objectStoreNames).join(", ")}]`)}catch(r){throw o("Error during database upgrade",r),r}},n.onblocked=()=>{r("IndexedDB upgrade blocked by another connection")}}),this.initPromise)}ensureInitialized(){if(!this.db)throw new StorageNotInitializedError("ensureInitialized");return this.db}async getStudent(e,t){const n=this.ensureInitialized(),s=E(e,t);return new Promise((e,t)=>{try{const o=n.transaction(C,"readonly"),r=o.objectStore(C).get(s);r.onsuccess=()=>{e(r.result||null)},r.onerror=()=>{t(new StorageError("Failed to get student record","getStudent",r.error))}}catch(o){t(new StorageError("Failed to get student record","getStudent",o))}})}async saveStudent(e){const t=this.ensureInitialized(),n=E(e.release,e.serviceId);return new Promise((s,o)=>{try{const r=t.transaction(C,"readwrite"),a=r.objectStore(C).put(e,n);a.onsuccess=()=>{s()},a.onerror=()=>{"QuotaExceededError"===a.error?.name?o(new StorageQuotaError("saveStudent")):o(new StorageError("Failed to save student record","saveStudent",a.error))},r.onerror=()=>{o(new StorageError("Transaction failed while saving student","saveStudent",r.error))}}catch(r){o(new StorageError("Failed to save student record","saveStudent",r))}})}async getStudentsByRelease(e){const t=this.ensureInitialized();return new Promise((n,s)=>{try{const o=t.transaction(C,"readonly").objectStore(C),r=o.index("by-release").getAll(e);r.onsuccess=()=>{n(r.result||[])},r.onerror=()=>{s(new StorageError("Failed to get students by release","getStudentsByRelease",r.error))}}catch(o){s(new StorageError("Failed to get students by release","getStudentsByRelease",o))}})}async clearAll(){const e=this.ensureInitialized();return new Promise((t,n)=>{try{const s=e.transaction([C,A],"readwrite"),o=s.objectStore(C),r=s.objectStore(A),a=o.clear(),d=r.clear();let c=!1,l=!1;a.onsuccess=()=>{c=!0,l&&t()},d.onsuccess=()=>{l=!0,c&&t()},a.onerror=()=>{n(new StorageError("Failed to clear students","clearAll",a.error))},d.onerror=()=>{n(new StorageError("Failed to clear backups","clearAll",d.error))},s.onerror=()=>{n(new StorageError("Transaction failed during clearAll","clearAll",s.error))}}catch(s){n(new StorageError("Failed to clear all data","clearAll",s))}})}async backup(e){const t=this.ensureInitialized(),n=(new Date).toISOString(),s=`backup_${n}_${e.serviceId}`,o=E(e.release,e.serviceId),r={...e,originalKey:o,timestamp:n};return new Promise((e,n)=>{try{const o=t.transaction(A,"readwrite"),a=o.objectStore(A).put(r,s);a.onsuccess=()=>{e()},a.onerror=()=>{"QuotaExceededError"===a.error?.name?n(new StorageQuotaError("backup")):n(new StorageError("Failed to create backup","backup",a.error))},o.onerror=()=>{n(new StorageError("Transaction failed during backup","backup",o.error))}}catch(o){n(new StorageError("Failed to create backup","backup",o))}})}close(){this.db&&(this.db.close(),this.db=null,this.initPromise=null)}}let q=null,T=null;function _(e,t){return 0===t||function(e){return 0===e.length}(e)?"unstarted":function(e,t){if(e.length!==t)return!1;return e.every(e=>!0===e.success)}(e,t)?"complete":"incomplete"}class StorageService{constructor(e="BrowserTest"){this.dbName=e,this.adapter=function(e=$){return q&&T!==e&&(q.close(),q=null),q||(q=new IndexedDBStorageAdapter(e),T=e),q}(e)}async init(){try{await this.adapter.init(),s(`Storage service initialized (IndexedDB "${this.dbName}" ready)`)}catch(e){throw o("Failed to initialize storage service",e),e}}async loadStudentRecord(e){try{const t=await this.adapter.getStudent(e.release,e.serviceId);if(t)return s(`Loaded student record for ${e.serviceId} from IndexedDB`),t;const n={schema:1,docId:e.release,release:e.release,serviceId:e.serviceId,name:e.name,attempted:0,correct:0,updated:(new Date).toISOString(),pages:{}};return s(`Created new student record for ${e.serviceId}`),n}catch(t){r(`IndexedDB error, creating new record: ${t.message}`);return{schema:1,docId:e.release,release:e.release,serviceId:e.serviceId,name:e.name,attempted:0,correct:0,updated:(new Date).toISOString(),pages:{}}}}async saveStudentRecord(e){try{e.updated=(new Date).toISOString();let t=0,n=0;for(const s of Object.values(e.pages)){const e=s.answers.filter(e=>""!==e.answer.trim());t+=e.length,n+=e.filter(e=>e.success).length}e.attempted=t,e.correct=n,await this.adapter.saveStudent(e),s(`Saved student record for ${e.serviceId} to IndexedDB`)}catch(t){throw o("Failed to save student record",t),t}}updateRecordWithAnswer(e,t,n,s,o){const r=e.pages[t]||{answers:[],state:"unstarted"};for(;r.answers.length<=n;)r.answers.push({answer:"",success:!1,timestamp:(new Date).toISOString()});r.answers[n]=s;const a=(new Date).toISOString();return r.firstAttempted||(r.firstAttempted=a),r.lastAttempted=a,r.state=_(r.answers,o),{...e,pages:{...e.pages,[t]:r}}}buildCache(e){return function(e){const t={totals:{total:0,answered:0,correct:0},pages:{}};for(const[n,s]of Object.entries(e.pages)){const e=u(0,s);t.pages[n]=e,t.totals.total+=e.total,t.totals.answered+=e.answered,t.totals.correct+=e.correct}return t}(e)}async getStudentsByRelease(e){try{return await this.adapter.getStudentsByRelease(e)}catch(t){throw o("Failed to get students by release",t),t}}async clearAll(){try{await this.adapter.clearAll(),s("Cleared all data from IndexedDB")}catch(e){throw o("Failed to clear all data",e),e}}async backup(e){try{await this.adapter.backup(e),s(`Created backup for ${e.serviceId}`)}catch(t){r(`Failed to create backup for ${e.serviceId}`,t)}}}let O=null,D=null;function P(e){if(O&&!e)return O;if(O&&e&&D!==e)return r(`Storage service already initialized with dbName="${D}", ignoring new dbName="${e}"`),O;if(!O){const t=e||"BrowserTest";O=new StorageService(t),D=t}return O}const j=Object.freeze(Object.defineProperty({__proto__:null,StorageService:StorageService,getStorageService:P},Symbol.toStringTag,{value:"Module"}));function B(e){return function(e,t="display"){if(null==e)return console.warn("Invalid date provided to formatTimestamp:",e),"Invalid Date";const n="string"==typeof e?new Date(e):e;return isNaN(n.getTime())?(console.warn("Invalid date provided to formatTimestamp:",e),"Invalid Date"):"csv"===t?function(e){return e.toISOString()}(n):function(e){return`${e.toLocaleDateString("en-US",{month:"short"})} ${e.getDate()} ${e.getHours().toString().padStart(2,"0")}:${e.getMinutes().toString().padStart(2,"0")}`}(n)}(e,"display")}const U=new WeakMap;function F(e,t){const n=U.get(e);let c;if(n){if(n.interactive||!t.interactive)return s("Quiz table already enhanced, skipping"),!0;s("Upgrading quiz table from non-interactive to interactive mode"),c=n.parsed}else c=a(e),c.errors&&c.errors.length>0&&o("Quiz table has validation errors:",c.errors);const u={parsed:c,interactive:t.interactive,pageId:t.pageId};if(t.interactive){if(!t.pageId)return o("Interactive mode requires pageId option"),!1;s(`Preparing interactive enhancement for pageId: ${t.pageId}`),u.debouncer=new Debouncer,u.inputs=[]}if(U.set(e,u),t.interactive){const t=function(e,t){const{parsed:n,pageId:a,debouncer:c}=t;if(!a||!c)return o("Interactive mode requires pageId and debouncer"),!1;(function(e){const t=e.querySelectorAll("thead th, thead td");t[1]&&b(t[1],"qd-hidden");const n=e.querySelectorAll("tbody tr");n.forEach(e=>{const t=e.querySelectorAll("td");t[1]&&b(t[1],"qd-hidden")})})(e),K(e);if(!w(l.SESSION))return o("No active session found"),!1;let u=w(l.CACHE);u?s(`Cache loaded: ${u.totals.total} total questions, ${Object.keys(u.pages).length} pages`):(s("No cache found, creating empty cache"),u={totals:{total:0,answered:0,correct:0},pages:{}});const h=n.questions.length;u=function(e,t,n){const s=e.pages[t];if(s&&s.total>=n)return e;const o=n-(s?.total||0),r={state:s?.state||"unstarted",total:n,answered:s?.answered||0,correct:s?.correct||0,last:s?.last,answers:s?.answers,analysis:s?.analysis};return{totals:{total:e.totals.total+o,answered:e.totals.answered,correct:e.totals.correct},pages:{...e.pages,[t]:r}}}(u,a,h),x(l.CACHE,u);const p=u?.pages[a],g=p?.answers||[];s(`Page ${a}: ${g.length} existing answers, state: ${p?.state||"none"}`);const y=e.querySelector("tbody");if(!y)return o("Quiz table has no tbody element"),!1;const S=Array.from(y.querySelectorAll("tr")),E=[];n.questions.forEach((n,a)=>{const c=S[a];if(!c)return;const u=Array.from(c.querySelectorAll("td"));if(3!==u.length)return;const h=u[0],p=u[1];if(!h||!p)return;const f=g[a];f&&f.answer&&s(`Q${a+1}: Pre-filling with "${f.answer}" (${f.success?"correct":"incorrect"})`);const b=function(e,t){if("mcq"===e.kind&&e.options){const n=m("select");n.className="qd-quiz-input";const s=m("option");return s.value="",s.textContent="Select an answer...",s.disabled=!0,n.appendChild(s),e.options.forEach((e,t)=>{const s=m("option");s.value=String(t+1),s.textContent=`${t+1}. ${e}`,n.appendChild(s)}),n.value=t?t.answer:"",n}{const e=m("input");return e.type="text",e.className="qd-quiz-input",e.placeholder="Enter value",t&&(e.value=t.answer),e}}(n,f);E.push(b),p.textContent="",p.appendChild(b),f&&V(p,f.success);const y="SELECT"===b.tagName?"change":"input";b.addEventListener(y,()=>{!function(e,t,n,a){const{debouncer:c,pageId:u,parsed:h}=t;if(!c||!u)return;const p=h.questions[n];if(!p)return;c.debounce(`save-answer-${n}`,()=>{!async function(e,t,n,a){const{pageId:c,parsed:u,inputs:h}=t;if(!c||!h)return;const p=u.questions[n];if(!p)return;const g=w(l.SESSION);if(!g)return void o("No active session found");const m=d(p,a),f={answer:a.trim(),success:m,timestamp:(new Date).toISOString()},b=P();let y;try{y=await b.loadStudentRecord(g)}catch(q){return void r("Failed to load student record, answer not saved",q)}const S=u.questions.length,E=b.updateRecordWithAnswer(y,c,n,f,S);try{await b.saveStudentRecord(E)}catch(q){r("Failed to save student record to IndexedDB",q)}const $=b.buildCache(E);x(l.CACHE,$);const C=e.querySelector(`tbody tr:nth-child(${n+1})`);if(C){const e=C.querySelector("td:nth-child(2)");e&&V(e,m)}v("qd:answer-saved",{pageId:c,answer:f});const A=E.pages[c];A&&v("qd:state-changed",{pageId:c,state:A.state});s(`Answer saved for question ${n+1} on page ${c}: ${m?"correct":"incorrect"}`)}(e,t,n,a)},200)}(e,t,a,b.value)})}),t.inputs=E;const $=()=>{J(e,t)},C=()=>{G(e)};document.addEventListener("qd:instructor-show-answers",$),document.addEventListener("qd:instructor-hide-answers",C);const A="true"===sessionStorage.getItem(l.INSTRUCTOR),q="true"===sessionStorage.getItem("qd/instructor/showAnswers");A&&q&&J(e,t);const T=()=>{e.querySelectorAll("td.qd-answer-correct, td.qd-answer-incorrect").forEach(e=>{b(e,"qd-answer-correct","qd-answer-incorrect")}),G(e),s("Cleared student UI state from quiz table on logout")};return document.addEventListener("qd:logout",T),t.cleanupInstructorListeners=()=>{document.removeEventListener("qd:instructor-show-answers",$),document.removeEventListener("qd:instructor-hide-answers",C),document.removeEventListener("qd:logout",T)},f(e,"qd-quiz-interactive"),s(`Quiz table enhanced in interactive mode for page ${a}`),!0}(e,u);return t?s(`Interactive enhancement succeeded for table with ${c.questions.length} questions`):o("Interactive enhancement failed"),t}return function(e){return function(e){const t=e.querySelector("colgroup");t&&t.remove()}(e),Q(e),K(e),f(e,"qd-quiz-non-interactive"),s("Quiz table enhanced in non-interactive mode"),!0}(e)}function V(e,t){b(e,"qd-answer-correct","qd-answer-incorrect"),f(e,t?"qd-answer-correct":"qd-answer-incorrect")}function Q(e){const t=e.querySelectorAll("thead th, thead td");t[1]&&f(t[1],"qd-hidden");e.querySelectorAll("tbody tr").forEach(e=>{const t=e.querySelectorAll("td");t[1]&&(f(t[1],"qd-hidden"),t[1].textContent="")})}function K(e){const t=e.querySelectorAll("thead th, thead td");t[2]&&f(t[2],"qd-hidden");e.querySelectorAll("tbody tr").forEach(e=>{const t=e.querySelectorAll("td");t[2]&&f(t[2],"qd-hidden")})}function W(e){return U.get(e)}async function J(e,t){const{pageId:n,parsed:r}=t;if(!n)return;const a=w(l.SESSION);if(!a)return;const{getStorageService:d}=await Promise.resolve().then(()=>j),c=d();try{const t=await c.getStudentsByRelease(a.release);if(0===t.length)return s("No student data available for this release"),void alert("No student data available for this release. Students need to log in and answer questions first.");const o=e.querySelector("tbody");if(!o)return;const d=Array.from(o.querySelectorAll("tr"));r.questions.forEach((e,s)=>{const o=d[s];if(!o)return;const r=Array.from(o.querySelectorAll("td"))[1];if(!r)return;const a=r.querySelector(".qd-student-answers");a&&a.remove();const c=[];if(t.forEach(e=>{const t=e.pages[n];if(!t||!t.answers)return;const o=t.answers[s];o&&c.push({name:e.name,serviceId:e.serviceId,answer:o.answer,success:o.success,timestamp:o.timestamp})}),c.length>0){const e=document.createElement("div");e.className="qd-student-answers",c.forEach(t=>{const n=document.createElement("div");n.className="qd-student-answer "+(t.success?"qd-correct":"qd-incorrect");const s=t.serviceId.slice(-4),o=B(t.timestamp);n.innerHTML=`\n            <span class="qd-student-name">${t.name} (${s})</span>:\n            <span class="qd-student-answer-text">${t.answer}</span>\n            <span class="qd-timestamp">${o}</span>\n          `,e.appendChild(n)}),r.appendChild(e)}}),s(`Displayed student answers for ${t.length} students on page ${n}`)}catch(u){o("Failed to load student answers",u)}}function G(e){e.querySelectorAll(".qd-student-answers").forEach(e=>e.remove()),s("Hid student answers from quiz table")}function Z(e,t=16){let n=5381;for(let o=0;o<e.length;o++){n=(n<<5)+n+e.charCodeAt(o),n&=n}const s=Math.abs(n).toString(16).padStart(8,"0");return s.repeat(Math.ceil(t/s.length)).substring(0,t)}function X(e){const t=h(e),n=t[0],s=n?p(n).length:0,o=e.className||"qd-analysis";return Z(`${t.length}x${s}:${o}`,16)}function Y(e,t,n){return`R${e}C${t}#f:${Z(n.replace(/\s+/g," ").trim(),8)}`}function ee(e){return e.classList.contains("interactive")}function te(e){const t=[];e.querySelector("tbody")||t.push("Analysis table must have a tbody element");const n=h(e);0===n.length&&t.push("Analysis table must have at least one row");const s=X(e),o=[];return n.forEach((e,t)=>{p(e).forEach((e,n)=>{if(ee(e)){const s=g(e),r=Y(t,n,s);o.push({row:t,col:n,key:r})}})}),{element:e,tableId:s,editableCells:o,errors:t.length>0?t:void 0}}const ne=new WeakMap;function se(e,t){const n=te(e);n.errors&&n.errors.length>0&&o("Analysis table has validation errors:",n.errors);const a={parsed:n,interactive:t.interactive,pageId:t.pageId};if(t.interactive){if(!t.pageId)return o("Interactive mode requires pageId option"),!1;a.debouncer=new Debouncer,a.cellKeyMap=new Map}return ne.set(e,a),t.interactive?function(e,t){const{parsed:n,pageId:a,debouncer:d,cellKeyMap:c}=t;if(!a||!d||!c)return o("Interactive mode requires pageId, debouncer, and cellKeyMap"),!1;if(!w(l.SESSION))return o("No active session found"),!1;const u=w(l.CACHE),m=u?.pages[a],b=m?.analysis,y=b?.cells||{},S=h(e);return n.editableCells.forEach(({row:e,col:n,key:a})=>{const d=S[e];if(!d)return;const u=p(d)[n];u&&(ee(u)?(c.set(u,a),y[a]&&(u.textContent=y[a]),u.contentEditable="true",f(u,"qd-editable"),u.addEventListener("input",()=>{!function(e,t,n){const{debouncer:a,pageId:d}=e;if(!a||!d)return;const c=g(t);a.debounce(`save-cell-${n}`,()=>{!async function(e,t,n){const{pageId:a,parsed:d}=e;if(!a)return;const c=w(l.SESSION);if(!c)return void o("No active session found");const u=P();let h;try{h=await u.loadStudentRecord(c)}catch(b){return void r("Failed to load student record, analysis not saved",b)}const p=h.pages[a]||{answers:[],state:"unstarted"},g=p.analysis||{tableId:d.tableId,cells:{}};g.cells[t]=n;const m=(new Date).toISOString();g.firstEdited||(g.firstEdited=m);g.lastEdited=m,p.analysis=g,h.pages[a]=p,h.updated=m;try{await u.saveStudentRecord(h)}catch(b){r("Failed to save student record to IndexedDB",b)}const f=u.buildCache(h);x(l.CACHE,f),v("qd:analysis-saved",{pageId:a,tableId:d.tableId,cellKey:t,content:n}),s(`Analysis cell saved for ${t} on page ${a}`)}(e,n,c)},500)}(t,u,a)})):o(`Cell at R${e}C${n} is no longer editable`))}),f(e,"qd-analysis-interactive"),s(`Analysis table enhanced in interactive mode for page ${a}`),!0}(e,a):function(e){f(e,"qd-analysis-non-interactive");const t=()=>{!async function(e){const t=ne.get(e);if(!t)return void r("Cannot show student entries: table not enhanced");const n=t.pageId||function(){const e=document.body.dataset.pageId;if(e)return e;const t=window.location.pathname,n=(t.split("/").pop()||"").replace(".html","");return n||void 0}();if(!n)return void r("Cannot show student entries: page ID not found");const a=w(l.SESSION);if(!a)return void r("Cannot show student entries: no active session");const d=P();let c;try{c=await d.getStudentsByRelease(a.release)}catch(f){return void o("Failed to load students for instructor view:",f)}const u=function(e,t){const n={};return e.forEach(e=>{const s=e.pages[t];if(!s||!s.analysis)return;const{cells:o}=s.analysis,r=s.analysis.lastEdited||e.updated;Object.entries(o).forEach(([t,s])=>{n[t]||(n[t]=[]),n[t].push({serviceId:e.serviceId,name:e.name,content:s,timestamp:r})})}),n}(c,n),{editableCells:g}=t.parsed,m=h(e);g.forEach(({row:e,col:t,key:n})=>{const s=m[e];if(!s)return;const o=p(s)[t];if(!o)return;const r=function(e){const t=document.createElement("div");if(t.className="qd-student-entries",0===e.length)return t.className+=" qd-no-entries",t.textContent="(No entries yet)",t.style.cssText="color: #9ca3af; font-style: italic; font-size: 13px; padding: 8px 0;",t;const n=function(e){return[...e].sort((e,t)=>{const n=new Date(e.timestamp).getTime();return new Date(t.timestamp).getTime()-n})}(e);return n.forEach(e=>{const n=document.createElement("div");n.className="qd-entry",n.style.cssText="padding: 8px 0; border-bottom: 1px solid #e5e7eb; font-size: 13px; color: #1f2937;";const s=e.serviceId.slice(-4),o=B(e.timestamp),r=document.createElement("span");r.style.cssText="font-weight: 600; color: #374151;",r.textContent=`${e.name} (${s}) • ${o}: `;const a=document.createElement("span");a.style.cssText="white-space: pre-wrap;",a.textContent=e.content,n.appendChild(r),n.appendChild(a),t.appendChild(n)}),t.style.cssText="margin-top: 12px; padding-top: 8px; border-top: 2px solid #3b82f6;",t}(u[n]||[]);r.setAttribute("data-qd-student-entries","true");const a=o.querySelector("[data-qd-student-entries]");a&&a.remove(),o.appendChild(r)}),s(`Displayed student entries for ${g.length} cells`)}(e)},n=()=>{oe(e)};return document.addEventListener("qd:instructor-show-answers",t),document.addEventListener("qd:instructor-hide-answers",n),s("Analysis table enhanced in non-interactive mode with instructor view support"),!0}(e)}function oe(e){e.querySelectorAll("[data-qd-student-entries]").forEach(e=>e.remove()),s("Hidden student entries from analysis table")}class EventCoordinator{constructor(){this.listeners=new Map}initialize(){this.registerLoginHandlers(),this.registerLogoutHandlers(),this.registerAnswerHandlers(),this.registerStateHandlers(),this.registerInstructorHandlers(),this.registerDataHandlers(),s("Event coordinator initialized")}registerLoginHandlers(){this.addEventListener("qd:login",e=>{(async()=>{const t=e.detail;s(`Login event: ${t.serviceId} (${t.name})`);const n=w(l.SESSION);if(!n)return void s("No session found in storage, skipping cache rebuild");const o=P();let r,a;try{r=await o.loadStudentRecord(n),await o.saveStudentRecord(r),a=o.buildCache(r),x(l.CACHE,a),s(`Cache built from IndexedDB: ${a.totals.total} total questions`)}catch{s("Failed to load from IndexedDB, initializing empty cache");x(l.CACHE,{totals:{total:0,answered:0,correct:0},pages:{}})}this.dispatchEvent("qd:cache-rebuild",{}),this.upgradeTablesAfterLogin()})()})}upgradeTablesAfterLogin(){const e=window.location.pathname,t=e.substring(e.lastIndexOf("/")+1).replace(/\.html?$/i,"");if(!t)return void s("No pageId found, skipping table upgrade to interactive mode");if("true"===sessionStorage.getItem(l.INSTRUCTOR)){s("Instructor session detected, tables remain in non-interactive mode with answers visible");return void document.querySelectorAll("table.qd-quiz").forEach(e=>{const n=W(e);if(!n)return;n.pageId=t;e.querySelectorAll("td:nth-child(2), th:nth-child(2)").forEach(e=>{e.classList.remove("qd-hidden")});e.querySelectorAll("tbody td:nth-child(2)").forEach((e,t)=>{const s=n.parsed.questions[t];s&&e instanceof HTMLTableCellElement&&(e.textContent=s.correctAnswer)});e.querySelectorAll("td:nth-child(3), th:nth-child(3)").forEach(e=>e.classList.remove("qd-hidden"));const s=()=>{J(e,n)};document.addEventListener("qd:instructor-show-answers",s),document.addEventListener("qd:instructor-hide-answers",()=>{G(e)});"true"===sessionStorage.getItem("qd/instructor/showAnswers")&&s()})}const n=document.querySelectorAll("table.qd-quiz");n.length>0&&(s(`Upgrading ${n.length} quiz table(s) to interactive mode...`),n.forEach(e=>{F(e,{interactive:!0,pageId:t})}));const o=document.querySelectorAll("table.qd-analysis");o.length>0&&(s(`Upgrading ${o.length} analysis table(s) to interactive mode...`),o.forEach(e=>{se(e,{interactive:!0,pageId:t})}))}registerLogoutHandlers(){this.addEventListener("qd:logout",e=>{s(`Logout event: ${e.detail.serviceId}`);document.querySelectorAll("table.qd-quiz").forEach(e=>{!function(e){const t=U.get(e);t&&(t.interactive=!1,t.pageId=void 0,t.inputs=void 0,t.cleanupInstructorListeners?.(),t.cleanupInstructorListeners=void 0,Q(e),K(e),b(e,"qd-quiz-interactive"),s("Quiz table reset to non-interactive mode"))}(e)});document.querySelectorAll("table.qd-analysis").forEach(e=>{!function(e){const t=ne.get(e);t&&(oe(e),t.interactive&&(e.querySelectorAll(".qd-editable").forEach(e=>{e instanceof HTMLTableCellElement&&(e.contentEditable="false",e.classList.remove("qd-editable"),e.textContent="")}),e.classList.remove("qd-analysis-interactive"),t.debouncer?.cancelAll()),t.interactive=!1,t.pageId=void 0,t.debouncer=void 0,t.cellKeyMap=void 0,s("Reset analysis table to non-interactive mode"))}(e)}),this.dispatchEvent("qd:cache-clear",{})})}registerAnswerHandlers(){this.addEventListener("qd:answer-saved",e=>{const t=e.detail;s(`Answer saved: ${t.pageId} Q${t.questionIndex} = ${t.answer} (${t.success?"correct":"incorrect"})`),this.dispatchEvent("qd:cache-update",{pageId:t.pageId})})}registerStateHandlers(){this.addEventListener("qd:state-changed",e=>{const t=e.detail;s(`State changed: ${t.pageId} → ${t.state}`),this.dispatchEvent("qd:badge-update",{pageId:t.pageId,state:t.state})})}registerInstructorHandlers(){this.addEventListener("qd:instructor-unlock",e=>{s(`Instructor mode unlocked at ${e.detail.unlockTime}`)}),this.addEventListener("qd:instructor-lock",()=>{s("Instructor mode locked")})}registerDataHandlers(){this.addEventListener("qd:data-cleared",e=>{s(`All data cleared at ${e.detail.timestamp}`),this.dispatchEvent("qd:cache-clear",{})})}addEventListener(e,t){document.addEventListener(e,t);const n=this.listeners.get(e)||[];n.push(t),this.listeners.set(e,n)}dispatchEvent(e,t){const n=new CustomEvent(e,{detail:t,bubbles:!0,composed:!0});document.dispatchEvent(n)}cleanup(){for(const[e,t]of this.listeners)for(const n of t)document.removeEventListener(e,n);this.listeners.clear(),s("Event coordinator cleaned up")}}class SessionCoordinator{constructor(){this.sessionService=new SessionService}initialize(){const e=this.sessionService.getSession();if(e){if(s(`Existing session loaded for ${e.serviceId}`),this.sessionService.isExpired())return r("Session expired, clearing"),void this.sessionService.clearSession();this.scheduleExpiryCheck(e),this.setupActivityTracking()}else s("No existing session found")}scheduleExpiryCheck(e){void 0!==this.expiryTimeoutId&&window.clearTimeout(this.expiryTimeoutId);const t=(new Date).getTime(),n=new Date(e.expiresAt).getTime()-t;n<=0?this.sessionService.clearSession():this.expiryTimeoutId=window.setTimeout(()=>{s("Session expired (timeout)"),this.sessionService.clearSession()},n)}setupActivityTracking(){const e=()=>{if(!this.sessionService.getSession())return;this.sessionService.updateActivity();const e=this.sessionService.getSession();e&&this.scheduleExpiryCheck(e)};let t;const n=()=>{void 0!==t&&window.clearTimeout(t),t=window.setTimeout(()=>{e()},5e3)};["click","keydown","scroll","mousemove"].forEach(e=>{document.addEventListener(e,n,{passive:!0})})}cleanup(){void 0!==this.expiryTimeoutId&&window.clearTimeout(this.expiryTimeoutId)}getSessionService(){return this.sessionService}}
/**
   * @license
   * Copyright 2019 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */const re=globalThis,ie=re.ShadowRoot&&(void 0===re.ShadyCSS||re.ShadyCSS.nativeShadow)&&"adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,ae=Symbol(),de=new WeakMap;let ce=class{constructor(e,t,n){if(this._$cssResult$=!0,n!==ae)throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=e,this.t=t}get styleSheet(){let e=this.o;const t=this.t;if(ie&&void 0===e){const n=void 0!==t&&1===t.length;n&&(e=de.get(t)),void 0===e&&((this.o=e=new CSSStyleSheet).replaceSync(this.cssText),n&&de.set(t,e))}return e}toString(){return this.cssText}};const le=(e,...t)=>{const n=1===e.length?e[0]:t.reduce((t,n,s)=>t+(e=>{if(!0===e._$cssResult$)return e.cssText;if("number"==typeof e)return e;throw Error("Value passed to 'css' function must be a 'css' function result: "+e+". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.")})(n)+e[s+1],e[0]);return new ce(n,e,ae)},ue=ie?e=>e:e=>e instanceof CSSStyleSheet?(e=>{let t="";for(const n of e.cssRules)t+=n.cssText;return(e=>new ce("string"==typeof e?e:e+"",void 0,ae))(t)})(e):e,{is:he,defineProperty:pe,getOwnPropertyDescriptor:ge,getOwnPropertyNames:me,getOwnPropertySymbols:fe,getPrototypeOf:be}=Object,ve=globalThis,ye=ve.trustedTypes,we=ye?ye.emptyScript:"",xe=ve.reactiveElementPolyfillSupport,Se=(e,t)=>e,Ee={toAttribute(e,t){switch(t){case Boolean:e=e?we:null;break;case Object:case Array:e=null==e?e:JSON.stringify(e)}return e},fromAttribute(e,t){let n=e;switch(t){case Boolean:n=null!==e;break;case Number:n=null===e?null:Number(e);break;case Object:case Array:try{n=JSON.parse(e)}catch(s){n=null}}return n}},$e=(e,t)=>!he(e,t),Ce={attribute:!0,type:String,converter:Ee,reflect:!1,useDefault:!1,hasChanged:$e};
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */Symbol.metadata??=Symbol("metadata"),ve.litPropertyMetadata??=new WeakMap;let Ie=class extends HTMLElement{static addInitializer(e){this._$Ei(),(this.l??=[]).push(e)}static get observedAttributes(){return this.finalize(),this._$Eh&&[...this._$Eh.keys()]}static createProperty(e,t=Ce){if(t.state&&(t.attribute=!1),this._$Ei(),this.prototype.hasOwnProperty(e)&&((t=Object.create(t)).wrapped=!0),this.elementProperties.set(e,t),!t.noAccessor){const n=Symbol(),s=this.getPropertyDescriptor(e,n,t);void 0!==s&&pe(this.prototype,e,s)}}static getPropertyDescriptor(e,t,n){const{get:s,set:o}=ge(this.prototype,e)??{get(){return this[t]},set(e){this[t]=e}};return{get:s,set(t){const r=s?.call(this);o?.call(this,t),this.requestUpdate(e,r,n)},configurable:!0,enumerable:!0}}static getPropertyOptions(e){return this.elementProperties.get(e)??Ce}static _$Ei(){if(this.hasOwnProperty(Se("elementProperties")))return;const e=be(this);e.finalize(),void 0!==e.l&&(this.l=[...e.l]),this.elementProperties=new Map(e.elementProperties)}static finalize(){if(this.hasOwnProperty(Se("finalized")))return;if(this.finalized=!0,this._$Ei(),this.hasOwnProperty(Se("properties"))){const e=this.properties,t=[...me(e),...fe(e)];for(const n of t)this.createProperty(n,e[n])}const e=this[Symbol.metadata];if(null!==e){const t=litPropertyMetadata.get(e);if(void 0!==t)for(const[e,n]of t)this.elementProperties.set(e,n)}this._$Eh=new Map;for(const[t,n]of this.elementProperties){const e=this._$Eu(t,n);void 0!==e&&this._$Eh.set(e,t)}this.elementStyles=this.finalizeStyles(this.styles)}static finalizeStyles(e){const t=[];if(Array.isArray(e)){const n=new Set(e.flat(1/0).reverse());for(const e of n)t.unshift(ue(e))}else void 0!==e&&t.push(ue(e));return t}static _$Eu(e,t){const n=t.attribute;return!1===n?void 0:"string"==typeof n?n:"string"==typeof e?e.toLowerCase():void 0}constructor(){super(),this._$Ep=void 0,this.isUpdatePending=!1,this.hasUpdated=!1,this._$Em=null,this._$Ev()}_$Ev(){this._$ES=new Promise(e=>this.enableUpdating=e),this._$AL=new Map,this._$E_(),this.requestUpdate(),this.constructor.l?.forEach(e=>e(this))}addController(e){(this._$EO??=new Set).add(e),void 0!==this.renderRoot&&this.isConnected&&e.hostConnected?.()}removeController(e){this._$EO?.delete(e)}_$E_(){const e=new Map,t=this.constructor.elementProperties;for(const n of t.keys())this.hasOwnProperty(n)&&(e.set(n,this[n]),delete this[n]);e.size>0&&(this._$Ep=e)}createRenderRoot(){const e=this.shadowRoot??this.attachShadow(this.constructor.shadowRootOptions);return((e,t)=>{if(ie)e.adoptedStyleSheets=t.map(e=>e instanceof CSSStyleSheet?e:e.styleSheet);else for(const n of t){const t=document.createElement("style"),s=re.litNonce;void 0!==s&&t.setAttribute("nonce",s),t.textContent=n.cssText,e.appendChild(t)}})(e,this.constructor.elementStyles),e}connectedCallback(){this.renderRoot??=this.createRenderRoot(),this.enableUpdating(!0),this._$EO?.forEach(e=>e.hostConnected?.())}enableUpdating(e){}disconnectedCallback(){this._$EO?.forEach(e=>e.hostDisconnected?.())}attributeChangedCallback(e,t,n){this._$AK(e,n)}_$ET(e,t){const n=this.constructor.elementProperties.get(e),s=this.constructor._$Eu(e,n);if(void 0!==s&&!0===n.reflect){const o=(void 0!==n.converter?.toAttribute?n.converter:Ee).toAttribute(t,n.type);this._$Em=e,null==o?this.removeAttribute(s):this.setAttribute(s,o),this._$Em=null}}_$AK(e,t){const n=this.constructor,s=n._$Eh.get(e);if(void 0!==s&&this._$Em!==s){const e=n.getPropertyOptions(s),o="function"==typeof e.converter?{fromAttribute:e.converter}:void 0!==e.converter?.fromAttribute?e.converter:Ee;this._$Em=s;const r=o.fromAttribute(t,e.type);this[s]=r??this._$Ej?.get(s)??r,this._$Em=null}}requestUpdate(e,t,n){if(void 0!==e){const s=this.constructor,o=this[e];if(n??=s.getPropertyOptions(e),!((n.hasChanged??$e)(o,t)||n.useDefault&&n.reflect&&o===this._$Ej?.get(e)&&!this.hasAttribute(s._$Eu(e,n))))return;this.C(e,t,n)}!1===this.isUpdatePending&&(this._$ES=this._$EP())}C(e,t,{useDefault:n,reflect:s,wrapped:o},r){n&&!(this._$Ej??=new Map).has(e)&&(this._$Ej.set(e,r??t??this[e]),!0!==o||void 0!==r)||(this._$AL.has(e)||(this.hasUpdated||n||(t=void 0),this._$AL.set(e,t)),!0===s&&this._$Em!==e&&(this._$Eq??=new Set).add(e))}async _$EP(){this.isUpdatePending=!0;try{await this._$ES}catch(t){Promise.reject(t)}const e=this.scheduleUpdate();return null!=e&&await e,!this.isUpdatePending}scheduleUpdate(){return this.performUpdate()}performUpdate(){if(!this.isUpdatePending)return;if(!this.hasUpdated){if(this.renderRoot??=this.createRenderRoot(),this._$Ep){for(const[e,t]of this._$Ep)this[e]=t;this._$Ep=void 0}const e=this.constructor.elementProperties;if(e.size>0)for(const[t,n]of e){const{wrapped:e}=n,s=this[t];!0!==e||this._$AL.has(t)||void 0===s||this.C(t,void 0,n,s)}}let e=!1;const t=this._$AL;try{e=this.shouldUpdate(t),e?(this.willUpdate(t),this._$EO?.forEach(e=>e.hostUpdate?.()),this.update(t)):this._$EM()}catch(n){throw e=!1,this._$EM(),n}e&&this._$AE(t)}willUpdate(e){}_$AE(e){this._$EO?.forEach(e=>e.hostUpdated?.()),this.hasUpdated||(this.hasUpdated=!0,this.firstUpdated(e)),this.updated(e)}_$EM(){this._$AL=new Map,this.isUpdatePending=!1}get updateComplete(){return this.getUpdateComplete()}getUpdateComplete(){return this._$ES}shouldUpdate(e){return!0}update(e){this._$Eq&&=this._$Eq.forEach(e=>this._$ET(e,this[e])),this._$EM()}updated(e){}firstUpdated(e){}};Ie.elementStyles=[],Ie.shadowRootOptions={mode:"open"},Ie[Se("elementProperties")]=new Map,Ie[Se("finalized")]=new Map,xe?.({ReactiveElement:Ie}),(ve.reactiveElementVersions??=[]).push("2.1.1");
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */
const Ae=globalThis,qe=Ae.trustedTypes,ke=qe?qe.createPolicy("lit-html",{createHTML:e=>e}):void 0,Te="$lit$",_e=`lit$${Math.random().toFixed(9).slice(2)}$`,Ne="?"+_e,ze=`<${Ne}>`,Oe=document,Le=()=>Oe.createComment(""),De=e=>null===e||"object"!=typeof e&&"function"!=typeof e,Pe=Array.isArray,Me="[ \t\n\f\r]",Re=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,je=/-->/g,Be=/>/g,He=RegExp(`>|${Me}(?:([^\\s"'>=/]+)(${Me}*=${Me}*(?:[^ \t\n\f\r"'\`<>=]|("|')|))|$)`,"g"),Ue=/'/g,Fe=/"/g,Ve=/^(?:script|style|textarea|title)$/i,Qe=(Ze=1,(e,...t)=>({_$litType$:Ze,strings:e,values:t})),Ke=Symbol.for("lit-noChange"),We=Symbol.for("lit-nothing"),Je=new WeakMap,Ge=Oe.createTreeWalker(Oe,129);var Ze;function Xe(e,t){if(!Pe(e)||!e.hasOwnProperty("raw"))throw Error("invalid template strings array");return void 0!==ke?ke.createHTML(t):t}class N{constructor({strings:e,_$litType$:t},n){let s;this.parts=[];let o=0,r=0;const a=e.length-1,d=this.parts,[c,l]=((e,t)=>{const n=e.length-1,s=[];let o,r=2===t?"<svg>":3===t?"<math>":"",a=Re;for(let d=0;d<n;d++){const t=e[d];let n,c,l=-1,u=0;for(;u<t.length&&(a.lastIndex=u,c=a.exec(t),null!==c);)u=a.lastIndex,a===Re?"!--"===c[1]?a=je:void 0!==c[1]?a=Be:void 0!==c[2]?(Ve.test(c[2])&&(o=RegExp("</"+c[2],"g")),a=He):void 0!==c[3]&&(a=He):a===He?">"===c[0]?(a=o??Re,l=-1):void 0===c[1]?l=-2:(l=a.lastIndex-c[2].length,n=c[1],a=void 0===c[3]?He:'"'===c[3]?Fe:Ue):a===Fe||a===Ue?a=He:a===je||a===Be?a=Re:(a=He,o=void 0);const h=a===He&&e[d+1].startsWith("/>")?" ":"";r+=a===Re?t+ze:l>=0?(s.push(n),t.slice(0,l)+Te+t.slice(l)+_e+h):t+_e+(-2===l?d:h)}return[Xe(e,r+(e[n]||"<?>")+(2===t?"</svg>":3===t?"</math>":"")),s]})(e,t);if(this.el=N.createElement(c,n),Ge.currentNode=this.el.content,2===t||3===t){const e=this.el.content.firstChild;e.replaceWith(...e.childNodes)}for(;null!==(s=Ge.nextNode())&&d.length<a;){if(1===s.nodeType){if(s.hasAttributes())for(const e of s.getAttributeNames())if(e.endsWith(Te)){const t=l[r++],n=s.getAttribute(e).split(_e),a=/([.?@])?(.*)/.exec(t);d.push({type:1,index:o,name:a[2],strings:n,ctor:"."===a[1]?H:"?"===a[1]?I:"@"===a[1]?L:k}),s.removeAttribute(e)}else e.startsWith(_e)&&(d.push({type:6,index:o}),s.removeAttribute(e));if(Ve.test(s.tagName)){const e=s.textContent.split(_e),t=e.length-1;if(t>0){s.textContent=qe?qe.emptyScript:"";for(let n=0;n<t;n++)s.append(e[n],Le()),Ge.nextNode(),d.push({type:2,index:++o});s.append(e[t],Le())}}}else if(8===s.nodeType)if(s.data===Ne)d.push({type:2,index:o});else{let e=-1;for(;-1!==(e=s.data.indexOf(_e,e+1));)d.push({type:7,index:o}),e+=_e.length-1}o++}}static createElement(e,t){const n=Oe.createElement("template");return n.innerHTML=e,n}}function Ye(e,t,n=e,s){if(t===Ke)return t;let o=void 0!==s?n._$Co?.[s]:n._$Cl;const r=De(t)?void 0:t._$litDirective$;return o?.constructor!==r&&(o?._$AO?.(!1),void 0===r?o=void 0:(o=new r(e),o._$AT(e,n,s)),void 0!==s?(n._$Co??=[])[s]=o:n._$Cl=o),void 0!==o&&(t=Ye(e,o._$AS(e,t.values),o,s)),t}class M{constructor(e,t){this._$AV=[],this._$AN=void 0,this._$AD=e,this._$AM=t}get parentNode(){return this._$AM.parentNode}get _$AU(){return this._$AM._$AU}u(e){const{el:{content:t},parts:n}=this._$AD,s=(e?.creationScope??Oe).importNode(t,!0);Ge.currentNode=s;let o=Ge.nextNode(),r=0,a=0,d=n[0];for(;void 0!==d;){if(r===d.index){let t;2===d.type?t=new R(o,o.nextSibling,this,e):1===d.type?t=new d.ctor(o,d.name,d.strings,this,e):6===d.type&&(t=new z(o,this,e)),this._$AV.push(t),d=n[++a]}r!==d?.index&&(o=Ge.nextNode(),r++)}return Ge.currentNode=Oe,s}p(e){let t=0;for(const n of this._$AV)void 0!==n&&(void 0!==n.strings?(n._$AI(e,n,t),t+=n.strings.length-2):n._$AI(e[t])),t++}}class R{get _$AU(){return this._$AM?._$AU??this._$Cv}constructor(e,t,n,s){this.type=2,this._$AH=We,this._$AN=void 0,this._$AA=e,this._$AB=t,this._$AM=n,this.options=s,this._$Cv=s?.isConnected??!0}get parentNode(){let e=this._$AA.parentNode;const t=this._$AM;return void 0!==t&&11===e?.nodeType&&(e=t.parentNode),e}get startNode(){return this._$AA}get endNode(){return this._$AB}_$AI(e,t=this){e=Ye(this,e,t),De(e)?e===We||null==e||""===e?(this._$AH!==We&&this._$AR(),this._$AH=We):e!==this._$AH&&e!==Ke&&this._(e):void 0!==e._$litType$?this.$(e):void 0!==e.nodeType?this.T(e):(e=>Pe(e)||"function"==typeof e?.[Symbol.iterator])(e)?this.k(e):this._(e)}O(e){return this._$AA.parentNode.insertBefore(e,this._$AB)}T(e){this._$AH!==e&&(this._$AR(),this._$AH=this.O(e))}_(e){this._$AH!==We&&De(this._$AH)?this._$AA.nextSibling.data=e:this.T(Oe.createTextNode(e)),this._$AH=e}$(e){const{values:t,_$litType$:n}=e,s="number"==typeof n?this._$AC(e):(void 0===n.el&&(n.el=N.createElement(Xe(n.h,n.h[0]),this.options)),n);if(this._$AH?._$AD===s)this._$AH.p(t);else{const e=new M(s,this),n=e.u(this.options);e.p(t),this.T(n),this._$AH=e}}_$AC(e){let t=Je.get(e.strings);return void 0===t&&Je.set(e.strings,t=new N(e)),t}k(e){Pe(this._$AH)||(this._$AH=[],this._$AR());const t=this._$AH;let n,s=0;for(const o of e)s===t.length?t.push(n=new R(this.O(Le()),this.O(Le()),this,this.options)):n=t[s],n._$AI(o),s++;s<t.length&&(this._$AR(n&&n._$AB.nextSibling,s),t.length=s)}_$AR(e=this._$AA.nextSibling,t){for(this._$AP?.(!1,!0,t);e!==this._$AB;){const t=e.nextSibling;e.remove(),e=t}}setConnected(e){void 0===this._$AM&&(this._$Cv=e,this._$AP?.(e))}}class k{get tagName(){return this.element.tagName}get _$AU(){return this._$AM._$AU}constructor(e,t,n,s,o){this.type=1,this._$AH=We,this._$AN=void 0,this.element=e,this.name=t,this._$AM=s,this.options=o,n.length>2||""!==n[0]||""!==n[1]?(this._$AH=Array(n.length-1).fill(new String),this.strings=n):this._$AH=We}_$AI(e,t=this,n,s){const o=this.strings;let r=!1;if(void 0===o)e=Ye(this,e,t,0),r=!De(e)||e!==this._$AH&&e!==Ke,r&&(this._$AH=e);else{const s=e;let a,d;for(e=o[0],a=0;a<o.length-1;a++)d=Ye(this,s[n+a],t,a),d===Ke&&(d=this._$AH[a]),r||=!De(d)||d!==this._$AH[a],d===We?e=We:e!==We&&(e+=(d??"")+o[a+1]),this._$AH[a]=d}r&&!s&&this.j(e)}j(e){e===We?this.element.removeAttribute(this.name):this.element.setAttribute(this.name,e??"")}}class H extends k{constructor(){super(...arguments),this.type=3}j(e){this.element[this.name]=e===We?void 0:e}}class I extends k{constructor(){super(...arguments),this.type=4}j(e){this.element.toggleAttribute(this.name,!!e&&e!==We)}}class L extends k{constructor(e,t,n,s,o){super(e,t,n,s,o),this.type=5}_$AI(e,t=this){if((e=Ye(this,e,t,0)??We)===Ke)return;const n=this._$AH,s=e===We&&n!==We||e.capture!==n.capture||e.once!==n.once||e.passive!==n.passive,o=e!==We&&(n===We||s);s&&this.element.removeEventListener(this.name,this,n),o&&this.element.addEventListener(this.name,this,e),this._$AH=e}handleEvent(e){"function"==typeof this._$AH?this._$AH.call(this.options?.host??this.element,e):this._$AH.handleEvent(e)}}class z{constructor(e,t,n){this.element=e,this.type=6,this._$AN=void 0,this._$AM=t,this.options=n}get _$AU(){return this._$AM._$AU}_$AI(e){Ye(this,e)}}const et=Ae.litHtmlPolyfillSupport;et?.(N,R),(Ae.litHtmlVersions??=[]).push("3.3.1");const tt=(e,t,n)=>{const s=n?.renderBefore??t;let o=s._$litPart$;if(void 0===o){const e=n?.renderBefore??null;s._$litPart$=o=new R(t.insertBefore(Le(),e),e,void 0,n??{})}return o._$AI(e),o},nt=globalThis;
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */class i extends Ie{constructor(){super(...arguments),this.renderOptions={host:this},this._$Do=void 0}createRenderRoot(){const e=super.createRenderRoot();return this.renderOptions.renderBefore??=e.firstChild,e}update(e){const t=this.render();this.hasUpdated||(this.renderOptions.isConnected=this.isConnected),super.update(e),this._$Do=tt(t,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this._$Do?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this._$Do?.setConnected(!1)}render(){return Ke}}i._$litElement$=!0,i.finalized=!0,nt.litElementHydrateSupport?.({LitElement:i});const st=nt.litElementPolyfillSupport;st?.({LitElement:i}),(nt.litElementVersions??=[]).push("4.2.1");
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */
const ot=e=>(t,n)=>{void 0!==n?n.addInitializer(()=>{customElements.define(e,t)}):customElements.define(e,t)},rt={attribute:!0,type:String,converter:Ee,reflect:!1,hasChanged:$e},it=(e=rt,t,n)=>{const{kind:s,metadata:o}=n;let r=globalThis.litPropertyMetadata.get(o);if(void 0===r&&globalThis.litPropertyMetadata.set(o,r=new Map),"setter"===s&&((e=Object.create(e)).wrapped=!0),r.set(n.name,e),"accessor"===s){const{name:s}=n;return{set(n){const o=t.get.call(this);t.set.call(this,n),this.requestUpdate(s,o,e)},init(t){return void 0!==t&&this.C(s,void 0,e,t),t}}}if("setter"===s){const{name:s}=n;return function(n){const o=this[s];t.call(this,n),this.requestUpdate(s,o,e)}}throw Error("Unsupported decorator location: "+s)};
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */function at(e){return(t,n)=>"object"==typeof n?it(e,t,n):((e,t,n)=>{const s=t.hasOwnProperty(n);return t.constructor.createProperty(n,e),s?Object.getOwnPropertyDescriptor(t,n):void 0})(e,t,n)}
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */function dt(e){return at({...e,state:!0,attribute:!1})}const ct=".wh_top_menu_and_indexterms_link",lt=".wh_publication_title .title",ut="",ht="BrowserTest",pt="qd-status-container",gt="qd-title-selector",mt="qd-instructor-hash",ft="qd-db-name";function bt(e,t){const n=document.querySelector(`#${e}`);if(!n)return t;const o=n.textContent?.trim()||"";return""===o?(r(`Config element #${e} found but empty, using default: "${t}"`),t):(s(`Config read from #${e}: "${o}"`),o)}var vt=Object.defineProperty,yt=Object.getOwnPropertyDescriptor,wt=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?yt(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&vt(t,n,r),r};let xt=class extends i{constructor(){super(...arguments),this.title="Sonar Quiz System",this.name="",this.serviceId="",this.instructorPassword="",this.showInstructorModal=!1,this.modalOverlay=null,this.modalErrorDiv=null,this.modalPasswordInput=null,this.errorMessage="",this.isSubmitting=!1,this.handleLogoutEvent=()=>{this.name="",this.serviceId="",this.instructorPassword="",this.errorMessage="",this.isSubmitting=!1,this.showInstructorModal=!1,this.cleanupModal(),this.updateVisibility()},this.handleEscape=e=>{"Escape"===e.key&&this.showInstructorModal&&this.closeInstructorModal()}}connectedCallback(){super.connectedCallback(),this.updateVisibility(),document.addEventListener("keydown",this.handleEscape),document.addEventListener("qd:logout",this.handleLogoutEvent)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("keydown",this.handleEscape),document.removeEventListener("qd:logout",this.handleLogoutEvent),this.cleanupModal()}cleanupModal(){this.modalOverlay&&(this.modalOverlay.remove(),this.modalOverlay=null),this.modalErrorDiv=null,this.modalPasswordInput=null}firstUpdated(){this.setAttribute("data-ready","")}updateVisibility(){w(l.SESSION)?this.removeAttribute("data-show"):this.setAttribute("data-show","")}render(){return Qe`
  ...
  `,$t([dt()],Ct.prototype,"total",2),$t([dt()],Ct.prototype,"correct",2),$t([dt()],Ct.prototype,"percentage",2),$t([dt()],Ct.prototype,"statusColor",2),Ct=$t([ot("qd-status")],Ct);const It=le`
  :host {
...
`;class RateLimiter{constructor(){this.failureCount=0,this.lockoutUntil=null}attempt(){return!(this.lockoutUntil&&Date.now()<this.lockoutUntil)&&(this.lockoutUntil&&Date.now()>=this.lockoutUntil&&(this.lockoutUntil=null),!0)}recordFailure(){this.failureCount++;const e=[2e3,4e3,8e3,16e3,3e4],t=e[Math.min(this.failureCount-1,e.length-1)]??3e4;this.lockoutUntil=Date.now()+t}reset(){this.failureCount=0,this.lockoutUntil=null}getRemainingSeconds(){if(!this.lockoutUntil)return 0;const e=Math.max(0,this.lockoutUntil-Date.now());return Math.ceil(e/1e3)}isLockedOut(){return null!==this.lockoutUntil&&Date.now()<this.lockoutUntil}}const At="instructor.password.hash";var qt=Object.defineProperty,kt=Object.getOwnPropertyDescriptor,Tt=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?kt(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&qt(t,n,r),r};let _t=class extends i{constructor(){super(...arguments),this.password="",this.error="",this.remainingSeconds=0,this.rateLimiter=new RateLimiter,this.handlePasswordInput=e=>{const t=e.target;this.password=t.value,this.error=""},this.handleSubmit=async e=>{e.preventDefault();if(!this.rateLimiter.attempt())return this.remainingSeconds=this.rateLimiter.getRemainingSeconds(),this.startCountdown(),void(this.error=`Too many attempts. Try again in ${this.remainingSeconds}s`);try{const e=function(){const e=document.getElementById(At);if(!e){const e=`Instructor password hash not found. Expected element with id="${At}". Check Oxygen XSL transform configuration.`;throw o(e),new Error(e)}const t=e.textContent?.trim();if(!t){const e="Instructor password hash element is empty. Check Oxygen parameter configuration.";throw o(e),new Error(e)}if(!/^[a-f0-9]{64}$/i.test(t)){const e=`Invalid password hash format. Expected 64 hex characters (SHA-256), got: ${t.substring(0,20)}...`;throw o(e),new Error(e)}return t.toLowerCase()}(),t=(new TextEncoder).encode(this.password),n=await crypto.subtle.digest("SHA-256",t),s=Array.from(new Uint8Array(n)).map(e=>e.toString(16).padStart(2,"0")).join(""),r=await async function(e,t){if(e.length!==t.length)return!1;if(0===e.length)return!0;const n=new TextEncoder,s=n.encode(e),o=n.encode(t);try{const e=await crypto.subtle.importKey("raw",s,{name:"HMAC",hash:"SHA-256"},!1,["sign"]),t=await crypto.subtle.sign("HMAC",e,o),n=await crypto.subtle.importKey("raw",o,{name:"HMAC",hash:"SHA-256"},!1,["sign"]),r=await crypto.subtle.sign("HMAC",n,s);if(t.byteLength!==r.byteLength)return!1;const a=new Uint8Array(t),d=new Uint8Array(r);let c=0;for(let s=0;s<a.length;s++)c|=(a[s]??0)^(d[s]??0);return 0===c}catch(r){return console.error("Constant-time comparison failed:",r),!1}}(s,e);r?(this.rateLimiter.reset(),this.password="",this.error="",y(this,"qd:instructor-unlock",{})):(this.error="Invalid password",this.password="")}catch{this.error="Authentication failed",this.password=""}}}disconnectedCallback(){super.disconnectedCallback(),this.countdownInterval&&window.clearInterval(this.countdownInterval)}startCountdown(){this.countdownInterval&&window.clearInterval(this.countdownInterval),this.countdownInterval=window.setInterval(()=>{this.remainingSeconds=this.rateLimiter.getRemainingSeconds(),0===this.remainingSeconds?(this.countdownInterval&&(window.clearInterval(this.countdownInterval),this.countdownInterval=void 0),this.error=""):this.error=`Too many attempts. Try again in ${this.remainingSeconds}s`},1e3)}render(){const e=this.remainingSeconds>0;return Qe`
    ...
    `}};Rt.styles=It,Mt([at({type:Array})],Rt.prototype,"students",2),Rt=Mt([ot("qd-instructor-export")],Rt);var jt=Object.defineProperty,Bt=Object.getOwnPropertyDescriptor,Ht=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?Bt(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&jt(t,n,r),r};let Ut=class extends i{constructor(){super(...arguments),this.showConfirmDialog=!1,this.confirmText="",this.error="",this.success="",this.modalContainer=null,this.handleClearRequest=()=>{this.showConfirmDialog=!0,this.confirmText="",this.error="",this.success=""},this.handleCancelClear=()=>{this.showConfirmDialog=!1,this.confirmText="",this.error=""},this.handleConfirmInput=e=>{const t=e.target;this.confirmText=t.value},this.handleConfirmClear=()=>{if("DELETE ALL DATA"===this.confirmText)try{S(),y(this,"qd:data-cleared",{}),this.success="All quiz data cleared successfully",this.showConfirmDialog=!1,this.confirmText="",this.error="",setTimeout(()=>{this.success=""},3e3)}catch{this.error="Failed to clear data"}else this.error="Confirmation text does not match"}}disconnectedCallback(){super.disconnectedCallback(),this.removeModalFromBody()}updated(e){super.updated(e),e.has("showConfirmDialog")&&(this.showConfirmDialog?this.renderModalToBody():this.removeModalFromBody()),this.showConfirmDialog&&(e.has("confirmText")||e.has("error"))&&this.renderModalToBody()}renderModalToBody(){this.modalContainer||(this.modalContainer=document.createElement("div"),this.modalContainer.className="qd-manage-modal-container",document.body.appendChild(this.modalContainer)),tt(this.renderConfirmDialog(),this.modalContainer)}removeModalFromBody(){this.modalContainer&&(this.modalContainer.remove(),this.modalContainer=null)}render(){return Qe`
      <button
    ...
    `}renderConfirmDialog(){const e="DELETE ALL DATA"===this.confirmText;return Qe`
      ...
      >
        ...
        >
          ...
          ${this.error?Qe`<div style="color: #dc3545; font-size: 14px; margin: 8px 0;">${this.error}</div>`:""}
    ...
    `],Qt([dt()],Kt.prototype,"unlocked",2),Qt([dt()],Kt.prototype,"showScores",2),Qt([dt()],Kt.prototype,"students",2),Qt([dt()],Kt.prototype,"showStudentAnswers",2),Kt=Qt([ot("qd-instructor")],Kt);var Wt=Object.defineProperty,Jt=Object.getOwnPropertyDescriptor,Gt=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?Jt(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&Wt(t,n,r),r};let Zt=class extends i{constructor(){super(...arguments),this.dbName="quiz-scores",this.hidden=!0,this.visible=!1,this.indexedDBEntries=[],this.sessionStorageEntries=[],this.handleToggleEntry=e=>{e.expanded=!e.expanded,this.requestUpdate()},this.handleClearSessionStorage=()=>{confirm("Clear all sessionStorage?")&&(sessionStorage.clear(),this.refreshData())},this.handleClearIndexedDB=async()=>{if(confirm(`Clear IndexedDB "${this.dbName}"?`))try{const e=await this.openDatabase();for(const t of Array.from(e.objectStoreNames)){const n=e.transaction(t,"readwrite");n.objectStore(t).clear()}await this.refreshData()}catch(e){console.error("Failed to clear IndexedDB:",e)}},this.handleClose=()=>{this.visible=!1,this.hidden=!0}}connectedCallback(){super.connectedCallback(),this.setupKeyboardShortcut(),this.startRefresh()}disconnectedCallback(){super.disconnectedCallback(),this.stopRefresh()}setupKeyboardShortcut(){document.addEventListener("keydown",e=>{e.ctrlKey&&e.shiftKey&&"D"===e.key&&(e.preventDefault(),this.toggleVisibility())})}toggleVisibility(){this.visible=!this.visible,this.hidden=!this.visible}startRefresh(){this.refreshData(),this.refreshInterval=window.setInterval(()=>{this.refreshData()},1e3)}stopRefresh(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async refreshData(){await this.refreshIndexedDB(),this.refreshSessionStorage()}async refreshIndexedDB(){try{const e=await this.openDatabase(),t=[];for(const n of Array.from(e.objectStoreNames)){const s=e.transaction(n,"readonly"),o=s.objectStore(n).getAll();await new Promise((e,s)=>{o.onsuccess=()=>{o.result.forEach((e,s)=>{t.push({key:`${n}[${s}]`,value:e,expanded:!1})}),e()},o.onerror=()=>s(new Error(o.error?.message||"IndexedDB request failed"))})}this.indexedDBEntries=t}catch{this.indexedDBEntries=[]}}refreshSessionStorage(){const e=[];for(let t=0;t<sessionStorage.length;t++){const n=sessionStorage.key(t);if(n)try{const t=sessionStorage.getItem(n);e.push({key:n,value:t?JSON.parse(t):t,expanded:!1})}catch{e.push({key:n,value:sessionStorage.getItem(n),expanded:!1})}}this.sessionStorageEntries=e}openDatabase(){return new Promise((e,t)=>{const n=indexedDB.open(this.dbName);n.onsuccess=()=>e(n.result),n.onerror=()=>t(new Error(n.error?.message||"Failed to open database"))})}renderEntry(e){return Qe`
      <div class="entry">
  ...
  `,Gt([at({type:String})],Zt.prototype,"dbName",2),Gt([at({type:Boolean,reflect:!0})],Zt.prototype,"hidden",2),Gt([dt()],Zt.prototype,"visible",2),Gt([dt()],Zt.prototype,"indexedDBEntries",2),Gt([dt()],Zt.prototype,"sessionStorageEntries",2),Zt=Gt([ot("qd-storage-monitor")],Zt);const Xt={statusPanel:".wh_top_menu_and_indexterms_link",storageMonitor:"body"};function Yt(e={}){const t=e.statusPanelContainer||Xt.statusPanel;!function(e){const t=document.querySelector(e);if(!t)return s(`Login component not injected: container '${e}' not found`),null;const n=document.createElement("qd-login");t.appendChild(n),s("Login component injected")}(t),function(e){const t=document.querySelector(e);if(!t)return s(`Status component not injected: container '${e}' not found`),null;const n=document.createElement("qd-status");t.appendChild(n),s("Status component injected")}(t),function(e){const t=document.querySelector(e);if(!t)return s(`Instructor component not injected: container '${e}' not found`),null;const n=document.createElement("qd-instructor");t.appendChild(n),s("Instructor component injected")}(t),function(e){if(!e.debug)return null;const t=e.storageMonitorContainer||Xt.storageMonitor,n=document.querySelector(t);if(!n)return s(`Storage monitor not injected: container '${t}' not found`),null;const o=document.createElement("qd-storage-monitor");e.dbName&&o.setAttribute("dbName",e.dbName),n.appendChild(o),s("Storage monitor injected (debug mode)")}({storageMonitorContainer:e.storageMonitorContainer,dbName:e.dbName,debug:e.debug})}const en={red:"qd-badge-red",amber:"qd-badge-amber",green:"qd-badge-green"},tn={unstarted:"red",incomplete:"amber",complete:"green"};function nn(e){const t=function(e,t){if(!e||!t?.pages)return"unstarted";const n=t.pages[e];return n?.state??"unstarted"}(e.getAttribute("data-page-id"),w(l.CACHE));!function(e,t){Object.values(en).forEach(t=>{e.classList.remove(t)});const n=en[tn[t]];e.classList.add(n)}(e,t)}function sn(){const e=document.querySelectorAll(".quizPageBtn"),t=w(l.CACHE),n="true"===sessionStorage.getItem(l.INSTRUCTOR);if(!t||n)return e.forEach(e=>{Object.values(en).forEach(t=>{e.classList.remove(t)})}),void s(n?`Removed badge styling from ${e.length} page links (instructor mode)`:`Removed badge styling from ${e.length} page links (no session)`);e.forEach(e=>{nn(e)}),s(`Updated ${e.length} page badges`)}function on(e){const t=e,{pageId:n}=t.detail,o=document.querySelector(`[data-page-id="${n}"]`);o&&o.classList.contains("quizPageBtn")&&(nn(o),s(`Updated badge for page ${n}`))}function rn(){s("Cache rebuilt, refreshing all badges"),sn()}function an(){s("Logout detected, removing all badge styling");const e=document.querySelectorAll(".quizPageBtn");e.forEach(e=>{Object.values(en).forEach(t=>{e.classList.remove(t)})}),s(`Removed badge styling from ${e.length} page links`)}const dn={initialized:!1};async function cn(e={}){if(dn.initialized)return void r("Bootstrap already initialized, skipping");s("Bootstrapping Sonar Quiz System..."),function(){if(document.getElementById("qd-global-styles"))return;const e=document.createElement("style");e.id="qd-global-styles",e.textContent="\n    /* Sonar Quiz System - Global Styles */\n    .qd-hidden {\n      display: none !important;\n    }\n\n    /* Quiz table interactive mode styles */\n    .qd-quiz-interactive .qd-quiz-input {\n      width: 100%;\n      padding: 0.5rem;\n      font-size: 1rem;\n      border: 1px solid #ccc;\n      border-radius: 4px;\n    }\n\n    /* Validation styling for answer cells */\n    .qd-quiz-interactive .qd-answer-correct {\n      background-color: #d4edda !important;\n      border-color: #28a745 !important;\n    }\n\n    .qd-quiz-interactive .qd-answer-incorrect {\n      background-color: #f8d7da !important;\n      border-color: #dc3545 !important;\n    }\n\n    /* Home page badge styles (R/A/G indicators) */\n    .qd-badge-red {\n      border-left: 4px solid #d32f2f !important;\n      background-color: #ffebee !important;\n    }\n\n    .qd-badge-amber {\n      border-left: 4px solid #ff9800 !important;\n      background-color: #fff3e0 !important;\n    }\n\n    .qd-badge-green {\n      border-left: 4px solid #4caf50 !important;\n      background-color: #e8f5e9 !important;\n    }\n\n    /* Instructor mode: Student answers display */\n    .qd-student-answers {\n      margin-top: 12px;\n      padding: 8px;\n      background: #f8f9fa;\n      border-radius: 4px;\n      border: 1px solid #dee2e6;\n    }\n\n    .qd-student-answer {\n      font-size: 12px;\n      padding: 4px 0;\n      line-height: 1.4;\n    }\n\n    .qd-student-answer.qd-correct {\n      color: #28a745;\n    }\n\n    .qd-student-answer.qd-incorrect {\n      color: #dc3545;\n    }\n\n    .qd-student-name {\n      font-weight: 600;\n    }\n\n    .qd-student-answer-text {\n      margin: 0 4px;\n    }\n\n    .qd-timestamp {\n      color: #6c757d;\n      font-size: 11px;\n      margin-left: 8px;\n    }\n  ",document.head.appendChild(e),s("Global styles injected")}();const t=P(e.dbName||"BrowserTest");await t.init();const n=new EventCoordinator;n.initialize(),dn.eventCoordinator=n;const o=new SessionCoordinator;o.initialize(),dn.sessionCoordinator=o,Yt({statusPanelContainer:e.statusPanelContainer,storageMonitorContainer:e.storageMonitorContainer,dbName:e.dbName,debug:e.debug}),!1!==e.autoEnhanceQuizTables&&function(){const e=document.querySelectorAll("table.qd-quiz");if(0===e.length)return void s("No quiz tables found to enhance");s(`Enhancing ${e.length} quiz table(s) in non-interactive mode...`);let t=0;for(const s of Array.from(e))try{F(s,{interactive:!1}),t++}catch(n){r(`Failed to enhance quiz table: ${n.message}`)}s(`Enhanced ${t} of ${e.length} quiz table(s) (non-interactive)`)}(),!1!==e.autoEnhanceAnalysisTables&&function(){const e=document.querySelectorAll("table.qd-analysis");if(0===e.length)return void s("No analysis tables found to enhance");s(`Enhancing ${e.length} analysis table(s) in non-interactive mode...`);let t=0;for(const s of Array.from(e))try{se(s,{interactive:!1}),t++}catch(n){r(`Failed to enhance analysis table: ${n.message}`)}s(`Enhanced ${t} of ${e.length} analysis table(s) (non-interactive)`)}(),!1!==e.autoEnhanceHomeBadges&&function(){const e=document.querySelectorAll(".quizPageBtn");if(0===e.length)return void s("No .quizPageBtn links found, skipping badge enhancement");s(`Enhancing home page badges for ${e.length} link(s)...`);try{document.querySelectorAll(".quizPageBtn").forEach(e=>{const t=function(e){const t=e.getAttribute("href");return t&&t.substring(t.lastIndexOf("/")+1).replace(/\.html?$/i,"")||null}(e);t?(e.setAttribute("data-page-id",t),s(`Set data-page-id="${t}" for link: ${e.textContent?.trim()}`)):s(`Failed to extract pageId from href: ${e.getAttribute("href")}`)}),sn(),document.addEventListener("qd:state-changed",on),document.addEventListener("qd:cache-rebuild",rn),document.addEventListener("qd:logout",an),s("Home page badges enhanced with event listeners"),s("Home page badges enhanced")}catch(t){r(`Failed to enhance home badges: ${t.message}`)}}(),await async function(){const e=w(l.SESSION);if(!e)return void s("No existing session, tables remain in non-interactive mode");if("true"===sessionStorage.getItem(l.INSTRUCTOR)){s("Instructor session detected, revealing answers in non-interactive tables");const e=window.location.pathname,t=e.substring(e.lastIndexOf("/")+1).replace(/\.html?$/i,"");return void document.querySelectorAll("table.qd-quiz").forEach(e=>{const n=W(e);if(!n)return;n.pageId=t;e.querySelectorAll("td:nth-child(2), th:nth-child(2)").forEach(e=>{e.classList.remove("qd-hidden")});e.querySelectorAll("tbody td:nth-child(2)").forEach((e,t)=>{const s=n.parsed.questions[t];s&&e instanceof HTMLTableCellElement&&(e.textContent=s.correctAnswer)});e.querySelectorAll("td:nth-child(3), th:nth-child(3)").forEach(e=>e.classList.remove("qd-hidden"));const s=()=>{J(e,n)},o=()=>{G(e)};document.addEventListener("qd:instructor-show-answers",s),document.addEventListener("qd:instructor-hide-answers",o);"true"===sessionStorage.getItem("qd/instructor/showAnswers")&&s()})}s(`Existing session detected for ${e.serviceId}, upgrading tables to interactive mode`);const t=P();let n=w(l.CACHE);if(!n){s("Cache not found, rebuilding from IndexedDB...");try{const o=await t.loadStudentRecord(e);n=t.buildCache(o),x(l.CACHE,n),s(`Cache rebuilt from IndexedDB: ${n.totals.total} total questions`)}catch{r("Failed to rebuild cache from IndexedDB, using empty cache"),n={totals:{total:0,answered:0,correct:0},pages:{}},x(l.CACHE,n)}}const o=window.location.pathname,a=o.substring(o.lastIndexOf("/")+1).replace(/\.html?$/i,"");if(!a)return void s("No pageId found, skipping table upgrade");const d=document.querySelectorAll("table.qd-quiz");d.length>0&&(s(`Upgrading ${d.length} quiz table(s) to interactive mode...`),d.forEach(e=>{F(e,{interactive:!0,pageId:a})}));const c=document.querySelectorAll("table.qd-analysis");c.length>0&&(s(`Upgrading ${c.length} analysis table(s) to interactive mode...`),c.forEach(e=>{se(e,{interactive:!0,pageId:a})}))}(),dn.initialized=!0,s("Bootstrap complete")}if("undefined"!=typeof window){const e=()=>{s("Auto-initializing Sonar Quiz System");const e=function(){s("Reading configuration from DOM...");const e={statusPanelContainer:bt(pt,ct),titleSelector:bt(gt,lt),instructorHash:bt(mt,ut),dbName:bt(ft,ht)};return s("Configuration loaded:",e),e}();cn({debug:true,dbName:e.dbName,statusPanelContainer:e.statusPanelContainer,autoEnhanceQuizTables:!0,autoEnhanceAnalysisTables:!0,autoEnhanceHomeBadges:!0})};"loading"===document.readyState?document.addEventListener("DOMContentLoaded",()=>{e()}):e()}return e.BUILD_DATE="2025-11-21",e.DEFAULT_CONTAINERS=Xt,e.Debouncer=Debouncer,e.SCHEMA_VERSION=1,e.SESSION_TIMEOUT_MS=c,e.STORAGE_KEYS=l,e.VERSION="0.1.0-phase3.1",e.bootstrap=cn,e.calculateCompletionState=_,e.cleanup=function(){dn.initialized?(s("Cleaning up bootstrap resources..."),dn.eventCoordinator?.cleanup(),dn.sessionCoordinator?.cleanup(),dn.initialized=!1,dn.eventCoordinator=void 0,dn.sessionCoordinator=void 0,s("Bootstrap cleanup complete")):r("Bootstrap not initialized, nothing to cleanup")},e.clearQuizData=S,e.enhanceAnalysisTable=se,e.enhanceQuizTable=F,e.error=o,e.generateCellKey=Y,e.generateTableId=X,e.getAnalysisTableMetadata=function(e){return ne.get(e)},e.getJSON=w,e.getQuizTableMetadata=W,e.info=s,e.injectComponents=Yt,e.isAnalysisTableEnhanced=function(e){return ne.has(e)},e.isCellEditable=ee,e.isInitialized=function(){return dn.initialized},e.isQuizTableEnhanced=function(e){return U.has(e)},e.parseAnalysisTable=te,e.parseQuizTable=a,e.setJSON=x,e.validateAnswer=d,e.warn=r,Object.defineProperty(e,Symbol.toStringTag,{value:"Module"}),e}({});
