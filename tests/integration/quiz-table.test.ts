...
describe('Quiz Table Enhancement', () => {
  ...
  describe('Interactive Mode', () => {
    ...
    it('should inject input controls when enhanced in interactive mode', () => {
      ...
      // Check that select controls are injected (MCQ questions use <select>)
      const selects = table.querySelectorAll('select.qd-quiz-input');
      expect(selects.length).toBe(2); // 2 questions
    ...
    it('should load existing answers into inputs', () => {
      ...
      // Check that first select has existing answer (MCQ uses <select>)
      const selects = table.querySelectorAll<HTMLSelectElement>('select.qd-quiz-input');
      expect(selects[0]?.value).toBe('1');
      expect(selects[1]?.value).toBe(''); // Second question not answered yet
    ...
    it('should save answer and emit events on user input', async () => {
      ...
      // Get first select and enter answer (MCQ uses <select>)
      const select = table.querySelector<HTMLSelectElement>('select.qd-quiz-input');
