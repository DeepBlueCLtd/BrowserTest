var SonarQuiz=function(e){"use strict";function t(e){if(e.length<2)return"**";if(2===e.length)return e;return e.slice(0,2)+"*".repeat(e.length-2)}function n(e){if(null===e||"object"!=typeof e)return e;const s={};for(const[o,r]of Object.entries(e))"name"!==o&&"passwordHash"!==o&&(s[o]="serviceId"!==o||"string"!=typeof r?"object"!=typeof r||null===r?r:n(r):t(r));return s}function s(e,t){void 0!==t?console.log(`[INFO] ${e}`,n(t)):console.log(`[INFO] ${e}`)}function o(e,t){if(t instanceof Error){const n={name:t.name,message:t.message};console.error(`[ERROR] ${e}`,n)}else void 0!==t?console.error(`[ERROR] ${e}`,n(t)):console.error(`[ERROR] ${e}`)}function r(e,t){void 0!==t?console.warn(`[WARN] ${e}`,n(t)):console.warn(`[WARN] ${e}`)}function a(e){const t=[],n=[];if(!e.classList.contains("qd-quiz"))return t.push('Table must have class "qd-quiz"'),{element:e,questions:n,errors:t};const s=Array.from(e.querySelectorAll("tbody tr"));return 0===s.length?(t.push("Quiz table has no data rows"),{element:e,questions:n,errors:t}):(s.forEach((e,s)=>{const o=Array.from(e.querySelectorAll("td"));if(3!==o.length)return void t.push(`Row ${s+1} has ${o.length} columns, expected 3 (Question | Answer | Detail)`);const r=o[0],a=o[1],d=o[2];if(!r||!a||!d)return;const c=r.textContent?.trim()||"";if(!c)return void t.push(`Row ${s+1} has empty question text`);const l=a.textContent?.trim()||"";if(!l)return void t.push(`Row ${s+1} has empty answer`);const u=d.querySelector("ol");if(u){const e=(h=u,Array.from(h.querySelectorAll("li")).map(e=>e.textContent?.trim()||"").filter(e=>e.length>0));if(0===e.length)return void t.push(`Row ${s+1} MCQ has no options in <ol>`);n.push({text:c,kind:"mcq",correctAnswer:l,options:e})}else{const e=d.textContent?.trim()||"",o=parseFloat(e);if(isNaN(o))return void t.push(`Row ${s+1} appears to be numeric but has invalid tolerance: "${e}"`);n.push({text:c,kind:"numeric",correctAnswer:l,tolerance:o})}var h}),{element:e,questions:n,errors:t.length>0?t:void 0})}function d(e,t){if(!t||""===t.trim())return!1;const n=t.trim();if("mcq"===e.kind)return n===e.correctAnswer;{const t=parseFloat(n),s=parseFloat(e.correctAnswer);if(isNaN(t)||isNaN(s))return!1;const o=e.tolerance??0;return Math.abs(t-s)<=o}}const c=18e5,l={SESSION:"qd/session",CACHE:"qd/state",INSTRUCTOR:"qd/instructor",PIN_ATTEMPTS:"qd:pin-attempts"},u=3,h=3e4;class SessionService{createSession(e,t,n){const o=new Date,r=o.toISOString(),a={serviceId:e,name:t,release:n,loginTime:r,lastActivity:r,expiresAt:new Date(o.getTime()+c).toISOString(),instructorUnlocked:!1};return this.saveSession(a),s(`Session created for ${e} (${t})`),this.emitEvent("qd:login",{serviceId:e,name:t,release:n,loginTime:r}),a}getSession(){try{const e=sessionStorage.getItem(l.SESSION);if(!e)return null;const t=JSON.parse(e);return t.serviceId&&t.release&&t.expiresAt?t:(r("Invalid session data, missing required fields"),null)}catch(e){return o("Failed to parse session data",e),null}}updateActivity(){const e=this.getSession();if(!e)return;const t=new Date;e.lastActivity=t.toISOString(),e.expiresAt=new Date(t.getTime()+c).toISOString(),this.saveSession(e)}isExpired(){const e=this.getSession();if(!e)return!0;return new Date>=new Date(e.expiresAt)}clearSession(){const e=this.getSession();sessionStorage.removeItem(l.SESSION),sessionStorage.removeItem(l.CACHE),sessionStorage.removeItem(l.INSTRUCTOR),sessionStorage.removeItem("qd/instructor/showAnswers"),e&&(s(`Session cleared for ${e.serviceId}`),this.emitEvent("qd:logout",{serviceId:e.serviceId,timestamp:(new Date).toISOString()}))}unlockInstructor(){const e=this.getSession();e&&(e.instructorUnlocked=!0,e.unlockTime=(new Date).toISOString(),this.saveSession(e),s("Instructor mode unlocked"),this.emitEvent("qd:instructor-unlock",{timestamp:e.unlockTime}))}lockInstructor(){const e=this.getSession();e&&(e.instructorUnlocked=!1,delete e.unlockTime,this.saveSession(e),s("Instructor mode locked"),this.emitEvent("qd:instructor-lock",{timestamp:(new Date).toISOString()}))}isInstructorUnlocked(){const e=this.getSession();return!0===e?.instructorUnlocked}getCache(){try{const e=sessionStorage.getItem(l.CACHE);return e?JSON.parse(e):null}catch(e){return o("Failed to parse cache data",e),null}}saveCache(e){try{sessionStorage.setItem(l.CACHE,JSON.stringify(e))}catch(t){o("Failed to save cache",t)}}clearCache(){sessionStorage.removeItem(l.CACHE)}saveSession(e){try{sessionStorage.setItem(l.SESSION,JSON.stringify(e))}catch(t){o("Failed to save session",t)}}emitEvent(e,t){try{const n=new CustomEvent(e,{detail:t,bubbles:!0});document.dispatchEvent(n)}catch(n){o(`Failed to emit event ${e}`,n)}}}function p(e,t){const n=t.answers.length,s=t.answers.filter(e=>""!==e.answer.trim()).length,o=t.answers.filter(e=>e.success).length;return{state:t.state,total:n,answered:s,correct:o,last:t.lastAttempted,answers:t.answers,analysis:t.analysis}}class Debouncer{constructor(){this.timers=new Map}debounce(e,t,n=200){const s=this.timers.get(e);void 0!==s&&clearTimeout(s);const o=setTimeout(()=>{this.timers.delete(e),t()},n);this.timers.set(e,o)}cancel(e){const t=this.timers.get(e);return void 0!==t&&(clearTimeout(t),this.timers.delete(e),!0)}cancelAll(){let e=0;for(const t of this.timers.values())clearTimeout(t),e++;return this.timers.clear(),e}isPending(e){return this.timers.has(e)}getPendingCount(){return this.timers.size}}function m(e){const t=e.querySelector("tbody");return t?Array.from(t.querySelectorAll("tr")):[]}function g(e){return Array.from(e.cells)}function f(e){return e&&e.textContent?.trim()||""}function b(e,t,n){return document.createElement(e)}function v(e,...t){e.classList.add(...t)}function y(e,...t){e.classList.remove(...t)}function w(e,t,n){const s=new CustomEvent(e,{detail:t,bubbles:!0,composed:!0,cancelable:!1});return document.dispatchEvent(s)}function x(e,t,n,s){const o=new CustomEvent(t,{detail:n,bubbles:!0,composed:!0,cancelable:!1});return e.dispatchEvent(o)}function S(e){try{const t=sessionStorage.getItem(e);return t?JSON.parse(t):null}catch(t){return r(`Failed to parse JSON from sessionStorage key: ${e}`,t),null}}function E(e,t){try{const n=JSON.stringify(t);return sessionStorage.setItem(e,n),!0}catch(n){return r(`Failed to store JSON in sessionStorage key: ${e}`,n),!1}}function C(){const e=[];for(let t=0;t<sessionStorage.length;t++){const n=sessionStorage.key(t);n&&n.startsWith("qd/")&&e.push(n)}for(const t of e)sessionStorage.removeItem(t);return e.length}function $(e,t){return`qd/${e}/u${t}`}class StorageError extends Error{constructor(e,t,n){super(e),this.operation=t,this.cause=n,this.name="StorageError",n?o(`Storage error in ${t}: ${e}`,n):o(`Storage error in ${t}: ${e}`)}}class StorageNotInitializedError extends StorageError{constructor(e){super("Storage adapter not initialized. Call init() first.",e),this.name="StorageNotInitializedError"}}class StorageQuotaError extends StorageError{constructor(e){super("Storage quota exceeded. Please clear old data or free up space.",e),this.name="StorageQuotaError"}}const A="students",q="backups",T="auditLog";class IndexedDBStorageAdapter{constructor(e){if(this.db=null,this.initPromise=null,!e)throw new Error("FATAL: dbName is required for IndexedDBStorageAdapter");this.dbName=e}async init(){return this.initPromise?this.initPromise:this.db?Promise.resolve():(this.initPromise=new Promise((e,t)=>{let n,s=!1;const a=()=>{n&&(clearTimeout(n),n=void 0)};n=window.setTimeout(()=>{if(s)return;s=!0,this.initPromise=null,r("IndexedDB open timed out after 5000ms - attempting recovery");const n=indexedDB.deleteDatabase(this.dbName);n.onsuccess=()=>{this.init().then(e).catch(t)},n.onerror=()=>{t(new StorageError(`Database "${this.dbName}" appears corrupted. Please clear site data in browser settings.`,"init"))},n.onblocked=()=>{t(new StorageError("Cannot recover database - close all other tabs with this site and reload.","init"))}},5e3);const d=indexedDB.open(this.dbName,3);d.onerror=()=>{s||(s=!0,a(),o(`IndexedDB open error: ${d.error?.message||"unknown"}`),this.initPromise=null,t(new StorageError("Failed to open database","init",d.error)))},d.onblocked=()=>{r("IndexedDB open blocked - close other tabs with this database")},d.onsuccess=()=>{if(!s){if(s=!0,a(),this.db=d.result,!this.db.objectStoreNames.contains(A)||!this.db.objectStoreNames.contains(q)||!this.db.objectStoreNames.contains(T)){r(`Database corrupted (missing stores). Found: [${Array.from(this.db.objectStoreNames).join(", ")}]`),this.db.close(),this.db=null;const n=indexedDB.deleteDatabase(this.dbName);return n.onsuccess=()=>{this.initPromise=null,this.init().then(e).catch(t)},void(n.onerror=()=>{this.initPromise=null,t(new StorageError("Failed to delete corrupted database","init",n.error))})}this.initPromise=null,e()}},d.onupgradeneeded=e=>{const t=e.target.result,n=e.target.transaction;n&&(n.onerror=()=>{o(`Upgrade transaction error: ${n.error?.message||"unknown"}`)},n.onabort=()=>{o(`Upgrade transaction aborted: ${n.error?.message||"unknown"}`)});try{if(!t.objectStoreNames.contains(A)){const e=t.createObjectStore(A,{keyPath:null});e.createIndex("by-release","release",{unique:!1}),e.createIndex("by-service-id","serviceId",{unique:!1})}if(!t.objectStoreNames.contains(q)){const e=t.createObjectStore(q,{keyPath:null});e.createIndex("by-original-key","originalKey",{unique:!1}),e.createIndex("by-timestamp","timestamp",{unique:!1})}if(!t.objectStoreNames.contains(T)){const e=t.createObjectStore(T,{keyPath:"eventId"});e.createIndex("by-service-id","serviceId",{unique:!1}),e.createIndex("by-reset-at","resetAt",{unique:!1})}}catch(s){throw o("Error during database upgrade",s),s}}}),this.initPromise)}ensureInitialized(){if(!this.db)throw new StorageNotInitializedError("ensureInitialized");return this.db}async getStudent(e,t){const n=this.ensureInitialized(),s=$(e,t);return new Promise((e,t)=>{try{const o=n.transaction(A,"readonly"),r=o.objectStore(A).get(s);r.onsuccess=()=>{e(r.result||null)},r.onerror=()=>{t(new StorageError("Failed to get student record","getStudent",r.error))}}catch(o){t(new StorageError("Failed to get student record","getStudent",o))}})}async saveStudent(e){const t=this.ensureInitialized(),n=$(e.release,e.serviceId);return new Promise((s,o)=>{try{const r=t.transaction(A,"readwrite"),a=r.objectStore(A).put(e,n);a.onsuccess=()=>{s()},a.onerror=()=>{"QuotaExceededError"===a.error?.name?o(new StorageQuotaError("saveStudent")):o(new StorageError("Failed to save student record","saveStudent",a.error))},r.onerror=()=>{o(new StorageError("Transaction failed while saving student","saveStudent",r.error))}}catch(r){o(new StorageError("Failed to save student record","saveStudent",r))}})}async getStudentsByRelease(e){const t=this.ensureInitialized();return new Promise((n,s)=>{try{const o=t.transaction(A,"readonly").objectStore(A),r=o.index("by-release").getAll(e);r.onsuccess=()=>{n(r.result||[])},r.onerror=()=>{s(new StorageError("Failed to get students by release","getStudentsByRelease",r.error))}}catch(o){s(new StorageError("Failed to get students by release","getStudentsByRelease",o))}})}async clearAll(){const e=this.ensureInitialized();return new Promise((t,n)=>{try{const s=e.transaction([A,q,T],"readwrite"),o=s.objectStore(A),r=s.objectStore(q),a=s.objectStore(T),d=o.clear(),c=r.clear(),l=a.clear();let u=!1,h=!1,p=!1;d.onsuccess=()=>{u=!0,h&&p&&t()},c.onsuccess=()=>{h=!0,u&&p&&t()},l.onsuccess=()=>{p=!0,u&&h&&t()},d.onerror=()=>{n(new StorageError("Failed to clear students","clearAll",d.error))},c.onerror=()=>{n(new StorageError("Failed to clear backups","clearAll",c.error))},l.onerror=()=>{n(new StorageError("Failed to clear audit log","clearAll",l.error))},s.onerror=()=>{n(new StorageError("Transaction failed during clearAll","clearAll",s.error))}}catch(s){n(new StorageError("Failed to clear all data","clearAll",s))}})}async backup(e){const t=this.ensureInitialized(),n=(new Date).toISOString(),s=`backup_${n}_${e.serviceId}`,o=$(e.release,e.serviceId),r={...e,originalKey:o,timestamp:n};return new Promise((e,n)=>{try{const o=t.transaction(q,"readwrite"),a=o.objectStore(q).put(r,s);a.onsuccess=()=>{e()},a.onerror=()=>{"QuotaExceededError"===a.error?.name?n(new StorageQuotaError("backup")):n(new StorageError("Failed to create backup","backup",a.error))},o.onerror=()=>{n(new StorageError("Transaction failed during backup","backup",o.error))}}catch(o){n(new StorageError("Failed to create backup","backup",o))}})}async saveAuditEvent(e){const t=this.ensureInitialized();return new Promise((n,s)=>{try{const o=t.transaction(T,"readwrite"),r=o.objectStore(T).add(e);r.onsuccess=()=>{n()},r.onerror=()=>{s(new StorageError("Failed to save audit event","saveAuditEvent",r.error))}}catch(o){s(new StorageError("Failed to save audit event","saveAuditEvent",o))}})}close(){this.db&&(this.db.close(),this.db=null,this.initPromise=null)}}let _=null,P=null;function O(e){if(!e)throw new Error("FATAL: dbName is required for getStorageAdapter()");return _&&P!==e&&(_.close(),_=null),_||(_=new IndexedDBStorageAdapter(e),P=e),_}function D(e,t){return 0===t||function(e){return 0===e.length}(e)?"unstarted":function(e,t){if(e.length!==t)return!1;return e.every(e=>!0===e.success)}(e,t)?"complete":"incomplete"}class StorageService{constructor(e){if(!e)throw new Error("FATAL: dbName is required for StorageService");this.dbName=e,this.adapter=O(e)}async init(){try{await this.adapter.init(),s(`Storage service initialized (IndexedDB "${this.dbName}" ready)`)}catch(e){throw o("Failed to initialize storage service",e),e}}async loadStudentRecord(e){try{const t=await this.adapter.getStudent(e.release,e.serviceId);if(t)return s(`Loaded student record for ${e.serviceId} from IndexedDB`),t;const n={schema:1,docId:e.release,release:e.release,serviceId:e.serviceId,name:e.name,attempted:0,correct:0,updated:(new Date).toISOString(),pages:{}};return s(`Created new student record for ${e.serviceId}`),n}catch(t){r(`IndexedDB error, creating new record: ${t.message}`);return{schema:1,docId:e.release,release:e.release,serviceId:e.serviceId,name:e.name,attempted:0,correct:0,updated:(new Date).toISOString(),pages:{}}}}async saveStudentRecord(e){try{e.updated=(new Date).toISOString();let t=0,n=0;for(const s of Object.values(e.pages)){const e=s.answers.filter(e=>""!==e.answer.trim());t+=e.length,n+=e.filter(e=>e.success).length}e.attempted=t,e.correct=n,await this.adapter.saveStudent(e),s(`Saved student record for ${e.serviceId} to IndexedDB`)}catch(t){throw o("Failed to save student record",t),t}}updateRecordWithAnswer(e,t,n,s,o){const r=e.pages[t]||{answers:[],state:"unstarted"};for(;r.answers.length<=n;)r.answers.push({answer:"",success:!1,timestamp:(new Date).toISOString()});r.answers[n]=s;const a=(new Date).toISOString();return r.firstAttempted||(r.firstAttempted=a),r.lastAttempted=a,r.state=D(r.answers,o),{...e,pages:{...e.pages,[t]:r}}}buildCache(e){return function(e){const t={totals:{total:0,answered:0,correct:0},pages:{}};for(const[n,s]of Object.entries(e.pages)){const e=p(0,s);t.pages[n]=e,t.totals.total+=e.total,t.totals.answered+=e.answered,t.totals.correct+=e.correct}return t}(e)}async getStudentsByRelease(e){try{return await this.adapter.getStudentsByRelease(e)}catch(t){throw o("Failed to get students by release",t),t}}async clearAll(){try{await this.adapter.clearAll(),s("Cleared all data from IndexedDB")}catch(e){throw o("Failed to clear all data",e),e}}async backup(e){try{await this.adapter.backup(e),s(`Created backup for ${e.serviceId}`)}catch(t){r(`Failed to create backup for ${e.serviceId}`,t)}}}let U=null,j=null;function B(e){if(U&&!e)return U;if(U&&e&&j!==e)return r(`Storage service already initialized with dbName="${j}", ignoring new dbName="${e}"`),U;if(!U){if(!e)throw new Error("FATAL: dbName is required for first getStorageService() call");U=new StorageService(e),j=e}return U}const F=Object.freeze(Object.defineProperty({__proto__:null,StorageService:StorageService,getStorageService:B},Symbol.toStringTag,{value:"Module"}));function V(e){return function(e,t="display"){if(null==e)return console.warn("Invalid date provided to formatTimestamp:",e),"Invalid Date";const n="string"==typeof e?new Date(e):e;return isNaN(n.getTime())?(console.warn("Invalid date provided to formatTimestamp:",e),"Invalid Date"):"csv"===t?function(e){return e.toISOString()}(n):function(e){return`${e.toLocaleDateString("en-US",{month:"short"})} ${e.getDate()} ${e.getHours().toString().padStart(2,"0")}:${e.getMinutes().toString().padStart(2,"0")}`}(n)}(e,"display")}const Q=new WeakMap;function W(e,t){const n=Q.get(e);let c;if(n){if(n.interactive||!t.interactive)return s("Quiz table already enhanced, skipping"),!0;s("Upgrading quiz table from non-interactive to interactive mode"),c=n.parsed}else c=a(e),c.errors&&c.errors.length>0&&o("Quiz table has validation errors:",c.errors);const u={parsed:c,interactive:t.interactive,pageId:t.pageId};if(t.interactive){if(!t.pageId)return o("Interactive mode requires pageId option"),!1;s(`Preparing interactive enhancement for pageId: ${t.pageId}`),u.debouncer=new Debouncer,u.inputs=[]}if(Q.set(e,u),t.interactive){const t=function(e,t){const{parsed:n,pageId:a,debouncer:c}=t;if(!a||!c)return o("Interactive mode requires pageId and debouncer"),!1;(function(e){const t=e.querySelectorAll("thead th, thead td");t[1]&&y(t[1],"qd-hidden");const n=e.querySelectorAll("tbody tr");n.forEach(e=>{const t=e.querySelectorAll("td");t[1]&&y(t[1],"qd-hidden")})})(e),G(e);if(!S(l.SESSION))return o("No active session found"),!1;let u=S(l.CACHE);u?s(`Cache loaded: ${u.totals.total} total questions, ${Object.keys(u.pages).length} pages`):(s("No cache found, creating empty cache"),u={totals:{total:0,answered:0,correct:0},pages:{}});const h=n.questions.length;u=function(e,t,n){const s=e.pages[t];if(s&&s.total>=n)return e;const o=n-(s?.total||0),r={state:s?.state||"unstarted",total:n,answered:s?.answered||0,correct:s?.correct||0,last:s?.last,answers:s?.answers,analysis:s?.analysis};return{totals:{total:e.totals.total+o,answered:e.totals.answered,correct:e.totals.correct},pages:{...e.pages,[t]:r}}}(u,a,h),E(l.CACHE,u);const p=u?.pages[a],m=p?.answers||[];s(`Page ${a}: ${m.length} existing answers, state: ${p?.state||"none"}`);const g=e.querySelector("tbody");if(!g)return o("Quiz table has no tbody element"),!1;const f=Array.from(g.querySelectorAll("tr")),x=[];n.questions.forEach((n,a)=>{const c=f[a];if(!c)return;const u=Array.from(c.querySelectorAll("td"));if(3!==u.length)return;const h=u[0],p=u[1];if(!h||!p)return;const g=m[a];g&&g.answer&&s(`Q${a+1}: Pre-filling with "${g.answer}" (${g.success?"correct":"incorrect"})`);const v=function(e,t){if("mcq"===e.kind&&e.options){const n=b("select");n.className="qd-quiz-input";const s=b("option");return s.value="",s.textContent="Select an answer...",s.disabled=!0,n.appendChild(s),e.options.forEach((e,t)=>{const s=b("option");s.value=String(t+1),s.textContent=`${t+1}. ${e}`,n.appendChild(s)}),n.value=t?t.answer:"",n}{const e=b("input");return e.type="text",e.className="qd-quiz-input",e.placeholder="Enter value",t&&(e.value=t.answer),e}}(n,g);x.push(v),p.textContent="",p.appendChild(v),g&&J(p,g.success);const y="SELECT"===v.tagName?"change":"input";v.addEventListener(y,()=>{!function(e,t,n,a){const{debouncer:c,pageId:u,parsed:h}=t;if(!c||!u)return;const p=h.questions[n];if(!p)return;c.debounce(`save-answer-${n}`,()=>{!async function(e,t,n,a){const{pageId:c,parsed:u,inputs:h}=t;if(!c||!h)return;const p=u.questions[n];if(!p)return;const m=S(l.SESSION);if(!m)return void o("No active session found");const g=d(p,a),f={answer:a.trim(),success:g,timestamp:(new Date).toISOString()},b=B();let v;try{v=await b.loadStudentRecord(m)}catch(q){return void r("Failed to load student record, answer not saved",q)}const y=u.questions.length,x=b.updateRecordWithAnswer(v,c,n,f,y);try{await b.saveStudentRecord(x)}catch(q){r("Failed to save student record to IndexedDB",q)}const C=b.buildCache(x);E(l.CACHE,C);const $=e.querySelector(`tbody tr:nth-child(${n+1})`);if($){const e=$.querySelector("td:nth-child(2)");e&&J(e,g)}w("qd:answer-saved",{pageId:c,answer:f});const A=x.pages[c];A&&w("qd:state-changed",{pageId:c,state:A.state});s(`Answer saved for question ${n+1} on page ${c}: ${g?"correct":"incorrect"}`)}(e,t,n,a)},200)}(e,t,a,v.value)})}),t.inputs=x;const C=()=>{Z(e,t)},$=()=>{X(e)};document.addEventListener("qd:instructor-show-answers",C),document.addEventListener("qd:instructor-hide-answers",$);const A="true"===sessionStorage.getItem(l.INSTRUCTOR),q="true"===sessionStorage.getItem("qd/instructor/showAnswers");A&&q&&Z(e,t);const T=()=>{e.querySelectorAll("td.qd-answer-correct, td.qd-answer-incorrect").forEach(e=>{y(e,"qd-answer-correct","qd-answer-incorrect")}),X(e),s("Cleared student UI state from quiz table on logout")};return document.addEventListener("qd:logout",T),t.cleanupInstructorListeners=()=>{document.removeEventListener("qd:instructor-show-answers",C),document.removeEventListener("qd:instructor-hide-answers",$),document.removeEventListener("qd:logout",T)},v(e,"qd-quiz-interactive"),s(`Quiz table enhanced in interactive mode for page ${a}`),!0}(e,u);return t?s(`Interactive enhancement succeeded for table with ${c.questions.length} questions`):o("Interactive enhancement failed"),t}return function(e){return function(e){const t=e.querySelector("colgroup");t&&t.remove()}(e),K(e),G(e),v(e,"qd-quiz-non-interactive"),s("Quiz table enhanced in non-interactive mode"),!0}(e)}function J(e,t){y(e,"qd-answer-correct","qd-answer-incorrect"),v(e,t?"qd-answer-correct":"qd-answer-incorrect")}function K(e){const t=e.querySelectorAll("thead th, thead td");t[1]&&v(t[1],"qd-hidden");e.querySelectorAll("tbody tr").forEach(e=>{const t=e.querySelectorAll("td");t[1]&&(v(t[1],"qd-hidden"),t[1].textContent="")})}function G(e){const t=e.querySelectorAll("thead th, thead td");t[2]&&v(t[2],"qd-hidden");e.querySelectorAll("tbody tr").forEach(e=>{const t=e.querySelectorAll("td");t[2]&&v(t[2],"qd-hidden")})}function Y(e){return Q.get(e)}async function Z(e,t){const{pageId:n,parsed:r}=t;if(!n)return;const a=S(l.SESSION);if(!a)return;const{getStorageService:d}=await Promise.resolve().then(()=>F),c=d();try{const t=await c.getStudentsByRelease(a.release);if(0===t.length)return s("No student data available for this release"),void alert("No student data available for this release. Students need to log in and answer questions first.");const o=e.querySelector("tbody");if(!o)return;const d=Array.from(o.querySelectorAll("tr"));r.questions.forEach((e,s)=>{const o=d[s];if(!o)return;const r=Array.from(o.querySelectorAll("td"))[1];if(!r)return;const a=r.querySelector(".qd-student-answers");a&&a.remove();const c=[];if(t.forEach(e=>{const t=e.pages[n];if(!t||!t.answers)return;const o=t.answers[s];o&&c.push({name:e.name,serviceId:e.serviceId,answer:o.answer,success:o.success,timestamp:o.timestamp})}),c.length>0){const e=document.createElement("div");e.className="qd-student-answers",c.forEach(t=>{const n=document.createElement("div");n.className="qd-student-answer "+(t.success?"qd-correct":"qd-incorrect");const s=t.serviceId.slice(-4),o=V(t.timestamp);n.innerHTML=`\n            <span class="qd-student-name">${t.name} (${s})</span>:\n            <span class="qd-student-answer-text">${t.answer}</span>\n            <span class="qd-timestamp">${o}</span>\n          `,e.appendChild(n)}),r.appendChild(e)}}),s(`Displayed student answers for ${t.length} students on page ${n}`)}catch(u){o("Failed to load student answers",u)}}function X(e){e.querySelectorAll(".qd-student-answers").forEach(e=>e.remove()),s("Hid student answers from quiz table")}function ee(e,t=16){let n=5381;for(let o=0;o<e.length;o++){n=(n<<5)+n+e.charCodeAt(o),n&=n}const s=Math.abs(n).toString(16).padStart(8,"0");return s.repeat(Math.ceil(t/s.length)).substring(0,t)}function te(e){const t=m(e),n=t[0],s=n?g(n).length:0,o=e.className||"qd-analysis";return ee(`${t.length}x${s}:${o}`,16)}function ne(e,t,n){return`R${e}C${t}#f:${ee(n.replace(/\s+/g," ").trim(),8)}`}function se(e){return e.classList.contains("interactive")}function oe(e){const t=[];e.querySelector("tbody")||t.push("Analysis table must have a tbody element");const n=m(e);0===n.length&&t.push("Analysis table must have at least one row");const s=te(e),o=[];return n.forEach((e,t)=>{g(e).forEach((e,n)=>{if(se(e)){const s=f(e),r=ne(t,n,s);o.push({row:t,col:n,key:r})}})}),{element:e,tableId:s,editableCells:o,errors:t.length>0?t:void 0}}const re=new WeakMap;function ie(e,t){const n=oe(e);n.errors&&n.errors.length>0&&o("Analysis table has validation errors:",n.errors);const a={parsed:n,interactive:t.interactive,pageId:t.pageId};if(t.interactive){if(!t.pageId)return o("Interactive mode requires pageId option"),!1;a.debouncer=new Debouncer,a.cellKeyMap=new Map}return re.set(e,a),t.interactive?function(e,t){const{parsed:n,pageId:a,debouncer:d,cellKeyMap:c}=t;if(!a||!d||!c)return o("Interactive mode requires pageId, debouncer, and cellKeyMap"),!1;if(!S(l.SESSION))return o("No active session found"),!1;const u=S(l.CACHE),h=u?.pages[a],p=h?.analysis,b=p?.cells||{},y=m(e);return n.editableCells.forEach(({row:e,col:n,key:a})=>{const d=y[e];if(!d)return;const u=g(d)[n];u&&(se(u)?(c.set(u,a),b[a]&&(u.textContent=b[a]),u.contentEditable="true",v(u,"qd-editable"),u.addEventListener("input",()=>{!function(e,t,n){const{debouncer:a,pageId:d}=e;if(!a||!d)return;const c=f(t);a.debounce(`save-cell-${n}`,()=>{!async function(e,t,n){const{pageId:a,parsed:d}=e;if(!a)return;const c=S(l.SESSION);if(!c)return void o("No active session found");const u=B();let h;try{h=await u.loadStudentRecord(c)}catch(b){return void r("Failed to load student record, analysis not saved",b)}const p=h.pages[a]||{answers:[],state:"unstarted"},m=p.analysis||{tableId:d.tableId,cells:{}};m.cells[t]=n;const g=(new Date).toISOString();m.firstEdited||(m.firstEdited=g);m.lastEdited=g,p.analysis=m,h.pages[a]=p,h.updated=g;try{await u.saveStudentRecord(h)}catch(b){r("Failed to save student record to IndexedDB",b)}const f=u.buildCache(h);E(l.CACHE,f),w("qd:analysis-saved",{pageId:a,tableId:d.tableId,cellKey:t,content:n}),s(`Analysis cell saved for ${t} on page ${a}`)}(e,n,c)},500)}(t,u,a)})):o(`Cell at R${e}C${n} is no longer editable`))}),v(e,"qd-analysis-interactive"),s(`Analysis table enhanced in interactive mode for page ${a}`),!0}(e,a):function(e){v(e,"qd-analysis-non-interactive");const t=()=>{!async function(e){const t=re.get(e);if(!t)return void r("Cannot show student entries: table not enhanced");const n=t.pageId||function(){const e=document.body.dataset.pageId;if(e)return e;const t=window.location.pathname,n=(t.split("/").pop()||"").replace(".html","");return n||void 0}();if(!n)return void r("Cannot show student entries: page ID not found");const a=S(l.SESSION);if(!a)return void r("Cannot show student entries: no active session");const d=B();let c;try{c=await d.getStudentsByRelease(a.release)}catch(f){return void o("Failed to load students for instructor view:",f)}const u=function(e,t){const n={};return e.forEach(e=>{const s=e.pages[t];if(!s||!s.analysis)return;const{cells:o}=s.analysis,r=s.analysis.lastEdited||e.updated;Object.entries(o).forEach(([t,s])=>{n[t]||(n[t]=[]),n[t].push({serviceId:e.serviceId,name:e.name,content:s,timestamp:r})})}),n}(c,n),{editableCells:h}=t.parsed,p=m(e);h.forEach(({row:e,col:t,key:n})=>{const s=p[e];if(!s)return;const o=g(s)[t];if(!o)return;const r=function(e){const t=document.createElement("div");if(t.className="qd-student-entries",0===e.length)return t.className+=" qd-no-entries",t.textContent="(No entries yet)",t.style.cssText="color: #9ca3af; font-style: italic; font-size: 13px; padding: 8px 0;",t;const n=function(e){return[...e].sort((e,t)=>{const n=new Date(e.timestamp).getTime();return new Date(t.timestamp).getTime()-n})}(e);return n.forEach(e=>{const n=document.createElement("div");n.className="qd-entry",n.style.cssText="padding: 8px 0; border-bottom: 1px solid #e5e7eb; font-size: 13px; color: #1f2937;";const s=e.serviceId.slice(-4),o=V(e.timestamp),r=document.createElement("span");r.style.cssText="font-weight: 600; color: #374151;",r.textContent=`${e.name} (${s}) • ${o}: `;const a=document.createElement("span");a.style.cssText="white-space: pre-wrap;",a.textContent=e.content,n.appendChild(r),n.appendChild(a),t.appendChild(n)}),t.style.cssText="margin-top: 12px; padding-top: 8px; border-top: 2px solid #3b82f6;",t}(u[n]||[]);r.setAttribute("data-qd-student-entries","true");const a=o.querySelector("[data-qd-student-entries]");a&&a.remove(),o.appendChild(r)}),s(`Displayed student entries for ${h.length} cells`)}(e)},n=()=>{ae(e)};return document.addEventListener("qd:instructor-show-answers",t),document.addEventListener("qd:instructor-hide-answers",n),s("Analysis table enhanced in non-interactive mode with instructor view support"),!0}(e)}function ae(e){e.querySelectorAll("[data-qd-student-entries]").forEach(e=>e.remove()),s("Hidden student entries from analysis table")}class EventCoordinator{constructor(){this.listeners=new Map}initialize(){this.registerLoginHandlers(),this.registerLogoutHandlers(),this.registerAnswerHandlers(),this.registerStateHandlers(),this.registerInstructorHandlers(),this.registerDataHandlers(),s("Event coordinator initialized")}registerLoginHandlers(){this.addEventListener("qd:login",e=>{(async()=>{const t=e.detail;if(s(`Login event: ${t.serviceId} (${t.name})`),"INSTRUCTOR"===t.serviceId)return void s("Instructor login - skipping student record handling");const n=S(l.SESSION);if(!n)return void s("No session found in storage, skipping cache rebuild");const o=B();let r,a;try{r=await o.loadStudentRecord(n),await o.saveStudentRecord(r),a=o.buildCache(r),E(l.CACHE,a),s(`Cache built from IndexedDB: ${a.totals.total} total questions`)}catch{s("Failed to load from IndexedDB, initializing empty cache");E(l.CACHE,{totals:{total:0,answered:0,correct:0},pages:{}})}this.dispatchEvent("qd:cache-rebuild",{}),this.upgradeTablesAfterLogin()})()})}upgradeTablesAfterLogin(){const e=window.location.pathname,t=e.substring(e.lastIndexOf("/")+1).replace(/\.html?$/i,"");if(!t)return void s("No pageId found, skipping table upgrade to interactive mode");if("true"===sessionStorage.getItem(l.INSTRUCTOR)){s("Instructor session detected, tables remain in non-interactive mode with answers visible");return void document.querySelectorAll("table.qd-quiz").forEach(e=>{const n=Y(e);if(!n)return;n.pageId=t;e.querySelectorAll("td:nth-child(2), th:nth-child(2)").forEach(e=>{e.classList.remove("qd-hidden")});e.querySelectorAll("tbody td:nth-child(2)").forEach((e,t)=>{const s=n.parsed.questions[t];s&&e instanceof HTMLTableCellElement&&(e.textContent=s.correctAnswer)});e.querySelectorAll("td:nth-child(3), th:nth-child(3)").forEach(e=>e.classList.remove("qd-hidden"));const s=()=>{Z(e,n)};document.addEventListener("qd:instructor-show-answers",s),document.addEventListener("qd:instructor-hide-answers",()=>{X(e)});"true"===sessionStorage.getItem("qd/instructor/showAnswers")&&s()})}const n=document.querySelectorAll("table.qd-quiz");n.length>0&&(s(`Upgrading ${n.length} quiz table(s) to interactive mode...`),n.forEach(e=>{W(e,{interactive:!0,pageId:t})}));const o=document.querySelectorAll("table.qd-analysis");o.length>0&&(s(`Upgrading ${o.length} analysis table(s) to interactive mode...`),o.forEach(e=>{ie(e,{interactive:!0,pageId:t})}))}registerLogoutHandlers(){this.addEventListener("qd:logout",e=>{s(`Logout event: ${e.detail.serviceId}`);document.querySelectorAll("table.qd-quiz").forEach(e=>{!function(e){const t=Q.get(e);t&&(t.interactive=!1,t.pageId=void 0,t.inputs=void 0,t.cleanupInstructorListeners?.(),t.cleanupInstructorListeners=void 0,K(e),G(e),y(e,"qd-quiz-interactive"),s("Quiz table reset to non-interactive mode"))}(e)});document.querySelectorAll("table.qd-analysis").forEach(e=>{!function(e){const t=re.get(e);t&&(ae(e),t.interactive&&(e.querySelectorAll(".qd-editable").forEach(e=>{e instanceof HTMLTableCellElement&&(e.contentEditable="false",e.classList.remove("qd-editable"),e.textContent="")}),e.classList.remove("qd-analysis-interactive"),t.debouncer?.cancelAll()),t.interactive=!1,t.pageId=void 0,t.debouncer=void 0,t.cellKeyMap=void 0,s("Reset analysis table to non-interactive mode"))}(e)}),this.dispatchEvent("qd:cache-clear",{})})}registerAnswerHandlers(){this.addEventListener("qd:answer-saved",e=>{const t=e.detail;s(`Answer saved: ${t.pageId} Q${t.questionIndex} = ${t.answer} (${t.success?"correct":"incorrect"})`),this.dispatchEvent("qd:cache-update",{pageId:t.pageId})})}registerStateHandlers(){this.addEventListener("qd:state-changed",e=>{const t=e.detail;s(`State changed: ${t.pageId} → ${t.state}`),this.dispatchEvent("qd:badge-update",{pageId:t.pageId,state:t.state})})}registerInstructorHandlers(){this.addEventListener("qd:instructor-unlock",e=>{s(`Instructor mode unlocked at ${e.detail.unlockTime}`)}),this.addEventListener("qd:instructor-lock",()=>{s("Instructor mode locked")})}registerDataHandlers(){this.addEventListener("qd:data-cleared",e=>{s(`All data cleared at ${e.detail.timestamp}`),this.dispatchEvent("qd:cache-clear",{})})}addEventListener(e,t){document.addEventListener(e,t);const n=this.listeners.get(e)||[];n.push(t),this.listeners.set(e,n)}dispatchEvent(e,t){const n=new CustomEvent(e,{detail:t,bubbles:!0,composed:!0});document.dispatchEvent(n)}cleanup(){for(const[e,t]of this.listeners)for(const n of t)document.removeEventListener(e,n);this.listeners.clear(),s("Event coordinator cleaned up")}}class SessionCoordinator{constructor(){this.sessionService=new SessionService}initialize(){const e=this.sessionService.getSession();if(e){if(s(`Existing session loaded for ${e.serviceId}`),this.sessionService.isExpired())return r("Session expired, clearing"),void this.sessionService.clearSession();this.scheduleExpiryCheck(e),this.setupActivityTracking()}else s("No existing session found")}scheduleExpiryCheck(e){void 0!==this.expiryTimeoutId&&window.clearTimeout(this.expiryTimeoutId);const t=(new Date).getTime(),n=new Date(e.expiresAt).getTime()-t;n<=0?this.sessionService.clearSession():this.expiryTimeoutId=window.setTimeout(()=>{s("Session expired (timeout)"),this.sessionService.clearSession()},n)}setupActivityTracking(){const e=()=>{if(!this.sessionService.getSession())return;this.sessionService.updateActivity();const e=this.sessionService.getSession();e&&this.scheduleExpiryCheck(e)};let t;const n=()=>{void 0!==t&&window.clearTimeout(t),t=window.setTimeout(()=>{e()},5e3)};["click","keydown","scroll","mousemove"].forEach(e=>{document.addEventListener(e,n,{passive:!0})})}cleanup(){void 0!==this.expiryTimeoutId&&window.clearTimeout(this.expiryTimeoutId)}getSessionService(){return this.sessionService}}
/**
   * @license
   * Copyright 2019 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */const de=globalThis,ce=de.ShadowRoot&&(void 0===de.ShadyCSS||de.ShadyCSS.nativeShadow)&&"adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,le=Symbol(),ue=new WeakMap;let he=class{constructor(e,t,n){if(this._$cssResult$=!0,n!==le)throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=e,this.t=t}get styleSheet(){let e=this.o;const t=this.t;if(ce&&void 0===e){const n=void 0!==t&&1===t.length;n&&(e=ue.get(t)),void 0===e&&((this.o=e=new CSSStyleSheet).replaceSync(this.cssText),n&&ue.set(t,e))}return e}toString(){return this.cssText}};const pe=(e,...t)=>{const n=1===e.length?e[0]:t.reduce((t,n,s)=>t+(e=>{if(!0===e._$cssResult$)return e.cssText;if("number"==typeof e)return e;throw Error("Value passed to 'css' function must be a 'css' function result: "+e+". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.")})(n)+e[s+1],e[0]);return new he(n,e,le)},me=ce?e=>e:e=>e instanceof CSSStyleSheet?(e=>{let t="";for(const n of e.cssRules)t+=n.cssText;return(e=>new he("string"==typeof e?e:e+"",void 0,le))(t)})(e):e,{is:ge,defineProperty:fe,getOwnPropertyDescriptor:be,getOwnPropertyNames:ve,getOwnPropertySymbols:ye,getPrototypeOf:we}=Object,xe=globalThis,Se=xe.trustedTypes,Ee=Se?Se.emptyScript:"",Ce=xe.reactiveElementPolyfillSupport,$e=(e,t)=>e,Ie={toAttribute(e,t){switch(t){case Boolean:e=e?Ee:null;break;case Object:case Array:e=null==e?e:JSON.stringify(e)}return e},fromAttribute(e,t){let n=e;switch(t){case Boolean:n=null!==e;break;case Number:n=null===e?null:Number(e);break;case Object:case Array:try{n=JSON.parse(e)}catch(s){n=null}}return n}},Ae=(e,t)=>!ge(e,t),qe={attribute:!0,type:String,converter:Ie,reflect:!1,useDefault:!1,hasChanged:Ae};
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */Symbol.metadata??=Symbol("metadata"),xe.litPropertyMetadata??=new WeakMap;let ke=class extends HTMLElement{static addInitializer(e){this._$Ei(),(this.l??=[]).push(e)}static get observedAttributes(){return this.finalize(),this._$Eh&&[...this._$Eh.keys()]}static createProperty(e,t=qe){if(t.state&&(t.attribute=!1),this._$Ei(),this.prototype.hasOwnProperty(e)&&((t=Object.create(t)).wrapped=!0),this.elementProperties.set(e,t),!t.noAccessor){const n=Symbol(),s=this.getPropertyDescriptor(e,n,t);void 0!==s&&fe(this.prototype,e,s)}}static getPropertyDescriptor(e,t,n){const{get:s,set:o}=be(this.prototype,e)??{get(){return this[t]},set(e){this[t]=e}};return{get:s,set(t){const r=s?.call(this);o?.call(this,t),this.requestUpdate(e,r,n)},configurable:!0,enumerable:!0}}static getPropertyOptions(e){return this.elementProperties.get(e)??qe}static _$Ei(){if(this.hasOwnProperty($e("elementProperties")))return;const e=we(this);e.finalize(),void 0!==e.l&&(this.l=[...e.l]),this.elementProperties=new Map(e.elementProperties)}static finalize(){if(this.hasOwnProperty($e("finalized")))return;if(this.finalized=!0,this._$Ei(),this.hasOwnProperty($e("properties"))){const e=this.properties,t=[...ve(e),...ye(e)];for(const n of t)this.createProperty(n,e[n])}const e=this[Symbol.metadata];if(null!==e){const t=litPropertyMetadata.get(e);if(void 0!==t)for(const[e,n]of t)this.elementProperties.set(e,n)}this._$Eh=new Map;for(const[t,n]of this.elementProperties){const e=this._$Eu(t,n);void 0!==e&&this._$Eh.set(e,t)}this.elementStyles=this.finalizeStyles(this.styles)}static finalizeStyles(e){const t=[];if(Array.isArray(e)){const n=new Set(e.flat(1/0).reverse());for(const e of n)t.unshift(me(e))}else void 0!==e&&t.push(me(e));return t}static _$Eu(e,t){const n=t.attribute;return!1===n?void 0:"string"==typeof n?n:"string"==typeof e?e.toLowerCase():void 0}constructor(){super(),this._$Ep=void 0,this.isUpdatePending=!1,this.hasUpdated=!1,this._$Em=null,this._$Ev()}_$Ev(){this._$ES=new Promise(e=>this.enableUpdating=e),this._$AL=new Map,this._$E_(),this.requestUpdate(),this.constructor.l?.forEach(e=>e(this))}addController(e){(this._$EO??=new Set).add(e),void 0!==this.renderRoot&&this.isConnected&&e.hostConnected?.()}removeController(e){this._$EO?.delete(e)}_$E_(){const e=new Map,t=this.constructor.elementProperties;for(const n of t.keys())this.hasOwnProperty(n)&&(e.set(n,this[n]),delete this[n]);e.size>0&&(this._$Ep=e)}createRenderRoot(){const e=this.shadowRoot??this.attachShadow(this.constructor.shadowRootOptions);return((e,t)=>{if(ce)e.adoptedStyleSheets=t.map(e=>e instanceof CSSStyleSheet?e:e.styleSheet);else for(const n of t){const t=document.createElement("style"),s=de.litNonce;void 0!==s&&t.setAttribute("nonce",s),t.textContent=n.cssText,e.appendChild(t)}})(e,this.constructor.elementStyles),e}connectedCallback(){this.renderRoot??=this.createRenderRoot(),this.enableUpdating(!0),this._$EO?.forEach(e=>e.hostConnected?.())}enableUpdating(e){}disconnectedCallback(){this._$EO?.forEach(e=>e.hostDisconnected?.())}attributeChangedCallback(e,t,n){this._$AK(e,n)}_$ET(e,t){const n=this.constructor.elementProperties.get(e),s=this.constructor._$Eu(e,n);if(void 0!==s&&!0===n.reflect){const o=(void 0!==n.converter?.toAttribute?n.converter:Ie).toAttribute(t,n.type);this._$Em=e,null==o?this.removeAttribute(s):this.setAttribute(s,o),this._$Em=null}}_$AK(e,t){const n=this.constructor,s=n._$Eh.get(e);if(void 0!==s&&this._$Em!==s){const e=n.getPropertyOptions(s),o="function"==typeof e.converter?{fromAttribute:e.converter}:void 0!==e.converter?.fromAttribute?e.converter:Ie;this._$Em=s;const r=o.fromAttribute(t,e.type);this[s]=r??this._$Ej?.get(s)??r,this._$Em=null}}requestUpdate(e,t,n){if(void 0!==e){const s=this.constructor,o=this[e];if(n??=s.getPropertyOptions(e),!((n.hasChanged??Ae)(o,t)||n.useDefault&&n.reflect&&o===this._$Ej?.get(e)&&!this.hasAttribute(s._$Eu(e,n))))return;this.C(e,t,n)}!1===this.isUpdatePending&&(this._$ES=this._$EP())}C(e,t,{useDefault:n,reflect:s,wrapped:o},r){n&&!(this._$Ej??=new Map).has(e)&&(this._$Ej.set(e,r??t??this[e]),!0!==o||void 0!==r)||(this._$AL.has(e)||(this.hasUpdated||n||(t=void 0),this._$AL.set(e,t)),!0===s&&this._$Em!==e&&(this._$Eq??=new Set).add(e))}async _$EP(){this.isUpdatePending=!0;try{await this._$ES}catch(t){Promise.reject(t)}const e=this.scheduleUpdate();return null!=e&&await e,!this.isUpdatePending}scheduleUpdate(){return this.performUpdate()}performUpdate(){if(!this.isUpdatePending)return;if(!this.hasUpdated){if(this.renderRoot??=this.createRenderRoot(),this._$Ep){for(const[e,t]of this._$Ep)this[e]=t;this._$Ep=void 0}const e=this.constructor.elementProperties;if(e.size>0)for(const[t,n]of e){const{wrapped:e}=n,s=this[t];!0!==e||this._$AL.has(t)||void 0===s||this.C(t,void 0,n,s)}}let e=!1;const t=this._$AL;try{e=this.shouldUpdate(t),e?(this.willUpdate(t),this._$EO?.forEach(e=>e.hostUpdate?.()),this.update(t)):this._$EM()}catch(n){throw e=!1,this._$EM(),n}e&&this._$AE(t)}willUpdate(e){}_$AE(e){this._$EO?.forEach(e=>e.hostUpdated?.()),this.hasUpdated||(this.hasUpdated=!0,this.firstUpdated(e)),this.updated(e)}_$EM(){this._$AL=new Map,this.isUpdatePending=!1}get updateComplete(){return this.getUpdateComplete()}getUpdateComplete(){return this._$ES}shouldUpdate(e){return!0}update(e){this._$Eq&&=this._$Eq.forEach(e=>this._$ET(e,this[e])),this._$EM()}updated(e){}firstUpdated(e){}};ke.elementStyles=[],ke.shadowRootOptions={mode:"open"},ke[$e("elementProperties")]=new Map,ke[$e("finalized")]=new Map,Ce?.({ReactiveElement:ke}),(xe.reactiveElementVersions??=[]).push("2.1.1");
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */
const Te=globalThis,_e=Te.trustedTypes,Le=_e?_e.createPolicy("lit-html",{createHTML:e=>e}):void 0,Ne="$lit$",Pe=`lit$${Math.random().toFixed(9).slice(2)}$`,ze="?"+Pe,Oe=`<${ze}>`,De=document,Me=()=>De.createComment(""),Re=e=>null===e||"object"!=typeof e&&"function"!=typeof e,He=Array.isArray,Ue="[ \t\n\f\r]",je=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,Be=/-->/g,Fe=/>/g,Ve=RegExp(`>|${Ue}(?:([^\\s"'>=/]+)(${Ue}*=${Ue}*(?:[^ \t\n\f\r"'\`<>=]|("|')|))|$)`,"g"),Qe=/'/g,We=/"/g,Je=/^(?:script|style|textarea|title)$/i,Ke=(et=1,(e,...t)=>({_$litType$:et,strings:e,values:t})),Ge=Symbol.for("lit-noChange"),Ye=Symbol.for("lit-nothing"),Ze=new WeakMap,Xe=De.createTreeWalker(De,129);var et;function tt(e,t){if(!He(e)||!e.hasOwnProperty("raw"))throw Error("invalid template strings array");return void 0!==Le?Le.createHTML(t):t}class N{constructor({strings:e,_$litType$:t},n){let s;this.parts=[];let o=0,r=0;const a=e.length-1,d=this.parts,[c,l]=((e,t)=>{const n=e.length-1,s=[];let o,r=2===t?"<svg>":3===t?"<math>":"",a=je;for(let d=0;d<n;d++){const t=e[d];let n,c,l=-1,u=0;for(;u<t.length&&(a.lastIndex=u,c=a.exec(t),null!==c);)u=a.lastIndex,a===je?"!--"===c[1]?a=Be:void 0!==c[1]?a=Fe:void 0!==c[2]?(Je.test(c[2])&&(o=RegExp("</"+c[2],"g")),a=Ve):void 0!==c[3]&&(a=Ve):a===Ve?">"===c[0]?(a=o??je,l=-1):void 0===c[1]?l=-2:(l=a.lastIndex-c[2].length,n=c[1],a=void 0===c[3]?Ve:'"'===c[3]?We:Qe):a===We||a===Qe?a=Ve:a===Be||a===Fe?a=je:(a=Ve,o=void 0);const h=a===Ve&&e[d+1].startsWith("/>")?" ":"";r+=a===je?t+Oe:l>=0?(s.push(n),t.slice(0,l)+Ne+t.slice(l)+Pe+h):t+Pe+(-2===l?d:h)}return[tt(e,r+(e[n]||"<?>")+(2===t?"</svg>":3===t?"</math>":"")),s]})(e,t);if(this.el=N.createElement(c,n),Xe.currentNode=this.el.content,2===t||3===t){const e=this.el.content.firstChild;e.replaceWith(...e.childNodes)}for(;null!==(s=Xe.nextNode())&&d.length<a;){if(1===s.nodeType){if(s.hasAttributes())for(const e of s.getAttributeNames())if(e.endsWith(Ne)){const t=l[r++],n=s.getAttribute(e).split(Pe),a=/([.?@])?(.*)/.exec(t);d.push({type:1,index:o,name:a[2],strings:n,ctor:"."===a[1]?H:"?"===a[1]?I:"@"===a[1]?L:k}),s.removeAttribute(e)}else e.startsWith(Pe)&&(d.push({type:6,index:o}),s.removeAttribute(e));if(Je.test(s.tagName)){const e=s.textContent.split(Pe),t=e.length-1;if(t>0){s.textContent=_e?_e.emptyScript:"";for(let n=0;n<t;n++)s.append(e[n],Me()),Xe.nextNode(),d.push({type:2,index:++o});s.append(e[t],Me())}}}else if(8===s.nodeType)if(s.data===ze)d.push({type:2,index:o});else{let e=-1;for(;-1!==(e=s.data.indexOf(Pe,e+1));)d.push({type:7,index:o}),e+=Pe.length-1}o++}}static createElement(e,t){const n=De.createElement("template");return n.innerHTML=e,n}}function nt(e,t,n=e,s){if(t===Ge)return t;let o=void 0!==s?n._$Co?.[s]:n._$Cl;const r=Re(t)?void 0:t._$litDirective$;return o?.constructor!==r&&(o?._$AO?.(!1),void 0===r?o=void 0:(o=new r(e),o._$AT(e,n,s)),void 0!==s?(n._$Co??=[])[s]=o:n._$Cl=o),void 0!==o&&(t=nt(e,o._$AS(e,t.values),o,s)),t}class M{constructor(e,t){this._$AV=[],this._$AN=void 0,this._$AD=e,this._$AM=t}get parentNode(){return this._$AM.parentNode}get _$AU(){return this._$AM._$AU}u(e){const{el:{content:t},parts:n}=this._$AD,s=(e?.creationScope??De).importNode(t,!0);Xe.currentNode=s;let o=Xe.nextNode(),r=0,a=0,d=n[0];for(;void 0!==d;){if(r===d.index){let t;2===d.type?t=new R(o,o.nextSibling,this,e):1===d.type?t=new d.ctor(o,d.name,d.strings,this,e):6===d.type&&(t=new z(o,this,e)),this._$AV.push(t),d=n[++a]}r!==d?.index&&(o=Xe.nextNode(),r++)}return Xe.currentNode=De,s}p(e){let t=0;for(const n of this._$AV)void 0!==n&&(void 0!==n.strings?(n._$AI(e,n,t),t+=n.strings.length-2):n._$AI(e[t])),t++}}class R{get _$AU(){return this._$AM?._$AU??this._$Cv}constructor(e,t,n,s){this.type=2,this._$AH=Ye,this._$AN=void 0,this._$AA=e,this._$AB=t,this._$AM=n,this.options=s,this._$Cv=s?.isConnected??!0}get parentNode(){let e=this._$AA.parentNode;const t=this._$AM;return void 0!==t&&11===e?.nodeType&&(e=t.parentNode),e}get startNode(){return this._$AA}get endNode(){return this._$AB}_$AI(e,t=this){e=nt(this,e,t),Re(e)?e===Ye||null==e||""===e?(this._$AH!==Ye&&this._$AR(),this._$AH=Ye):e!==this._$AH&&e!==Ge&&this._(e):void 0!==e._$litType$?this.$(e):void 0!==e.nodeType?this.T(e):(e=>He(e)||"function"==typeof e?.[Symbol.iterator])(e)?this.k(e):this._(e)}O(e){return this._$AA.parentNode.insertBefore(e,this._$AB)}T(e){this._$AH!==e&&(this._$AR(),this._$AH=this.O(e))}_(e){this._$AH!==Ye&&Re(this._$AH)?this._$AA.nextSibling.data=e:this.T(De.createTextNode(e)),this._$AH=e}$(e){const{values:t,_$litType$:n}=e,s="number"==typeof n?this._$AC(e):(void 0===n.el&&(n.el=N.createElement(tt(n.h,n.h[0]),this.options)),n);if(this._$AH?._$AD===s)this._$AH.p(t);else{const e=new M(s,this),n=e.u(this.options);e.p(t),this.T(n),this._$AH=e}}_$AC(e){let t=Ze.get(e.strings);return void 0===t&&Ze.set(e.strings,t=new N(e)),t}k(e){He(this._$AH)||(this._$AH=[],this._$AR());const t=this._$AH;let n,s=0;for(const o of e)s===t.length?t.push(n=new R(this.O(Me()),this.O(Me()),this,this.options)):n=t[s],n._$AI(o),s++;s<t.length&&(this._$AR(n&&n._$AB.nextSibling,s),t.length=s)}_$AR(e=this._$AA.nextSibling,t){for(this._$AP?.(!1,!0,t);e!==this._$AB;){const t=e.nextSibling;e.remove(),e=t}}setConnected(e){void 0===this._$AM&&(this._$Cv=e,this._$AP?.(e))}}class k{get tagName(){return this.element.tagName}get _$AU(){return this._$AM._$AU}constructor(e,t,n,s,o){this.type=1,this._$AH=Ye,this._$AN=void 0,this.element=e,this.name=t,this._$AM=s,this.options=o,n.length>2||""!==n[0]||""!==n[1]?(this._$AH=Array(n.length-1).fill(new String),this.strings=n):this._$AH=Ye}_$AI(e,t=this,n,s){const o=this.strings;let r=!1;if(void 0===o)e=nt(this,e,t,0),r=!Re(e)||e!==this._$AH&&e!==Ge,r&&(this._$AH=e);else{const s=e;let a,d;for(e=o[0],a=0;a<o.length-1;a++)d=nt(this,s[n+a],t,a),d===Ge&&(d=this._$AH[a]),r||=!Re(d)||d!==this._$AH[a],d===Ye?e=Ye:e!==Ye&&(e+=(d??"")+o[a+1]),this._$AH[a]=d}r&&!s&&this.j(e)}j(e){e===Ye?this.element.removeAttribute(this.name):this.element.setAttribute(this.name,e??"")}}class H extends k{constructor(){super(...arguments),this.type=3}j(e){this.element[this.name]=e===Ye?void 0:e}}class I extends k{constructor(){super(...arguments),this.type=4}j(e){this.element.toggleAttribute(this.name,!!e&&e!==Ye)}}class L extends k{constructor(e,t,n,s,o){super(e,t,n,s,o),this.type=5}_$AI(e,t=this){if((e=nt(this,e,t,0)??Ye)===Ge)return;const n=this._$AH,s=e===Ye&&n!==Ye||e.capture!==n.capture||e.once!==n.once||e.passive!==n.passive,o=e!==Ye&&(n===Ye||s);s&&this.element.removeEventListener(this.name,this,n),o&&this.element.addEventListener(this.name,this,e),this._$AH=e}handleEvent(e){"function"==typeof this._$AH?this._$AH.call(this.options?.host??this.element,e):this._$AH.handleEvent(e)}}class z{constructor(e,t,n){this.element=e,this.type=6,this._$AN=void 0,this._$AM=t,this.options=n}get _$AU(){return this._$AM._$AU}_$AI(e){nt(this,e)}}const st=Te.litHtmlPolyfillSupport;st?.(N,R),(Te.litHtmlVersions??=[]).push("3.3.1");const ot=(e,t,n)=>{const s=n?.renderBefore??t;let o=s._$litPart$;if(void 0===o){const e=n?.renderBefore??null;s._$litPart$=o=new R(t.insertBefore(Me(),e),e,void 0,n??{})}return o._$AI(e),o},rt=globalThis;
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */class i extends ke{constructor(){super(...arguments),this.renderOptions={host:this},this._$Do=void 0}createRenderRoot(){const e=super.createRenderRoot();return this.renderOptions.renderBefore??=e.firstChild,e}update(e){const t=this.render();this.hasUpdated||(this.renderOptions.isConnected=this.isConnected),super.update(e),this._$Do=ot(t,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this._$Do?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this._$Do?.setConnected(!1)}render(){return Ge}}i._$litElement$=!0,i.finalized=!0,rt.litElementHydrateSupport?.({LitElement:i});const it=rt.litElementPolyfillSupport;it?.({LitElement:i}),(rt.litElementVersions??=[]).push("4.2.1");
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */
const at=e=>(t,n)=>{void 0!==n?n.addInitializer(()=>{customElements.define(e,t)}):customElements.define(e,t)},dt={attribute:!0,type:String,converter:Ie,reflect:!1,hasChanged:Ae},ct=(e=dt,t,n)=>{const{kind:s,metadata:o}=n;let r=globalThis.litPropertyMetadata.get(o);if(void 0===r&&globalThis.litPropertyMetadata.set(o,r=new Map),"setter"===s&&((e=Object.create(e)).wrapped=!0),r.set(n.name,e),"accessor"===s){const{name:s}=n;return{set(n){const o=t.get.call(this);t.set.call(this,n),this.requestUpdate(s,o,e)},init(t){return void 0!==t&&this.C(s,void 0,e,t),t}}}if("setter"===s){const{name:s}=n;return function(n){const o=this[s];t.call(this,n),this.requestUpdate(s,o,e)}}throw Error("Unsupported decorator location: "+s)};
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */function lt(e){return(t,n)=>"object"==typeof n?ct(e,t,n):((e,t,n)=>{const s=t.hasOwnProperty(n);return t.constructor.createProperty(n,e),s?Object.getOwnPropertyDescriptor(t,n):void 0})(e,t,n)}
/**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */function ut(e){return lt({...e,state:!0,attribute:!1})}const ht=".wh_top_menu_and_indexterms_link",pt=".wh_publication_title .title",mt="",gt="qd-status-container",ft="qd-title-selector",bt="qd-instructor-hash",vt="qd-db-name";function yt(e,t){const n=document.querySelector(`#${e}`);if(!n)return t;const o=n.textContent?.trim()||"";return""===o?(r(`Config element #${e} found but empty, using default: "${t}"`),t):(s(`Config read from #${e}: "${o}"`),o)}function wt(){s("Reading configuration from DOM...");const e=function(e){const t=document.querySelector(`#${e}`);if(!t){const t=`FATAL: Required config element #${e} not found in DOM. Processing stopped.`;throw console.error(t),new Error(t)}const n=t.textContent?.trim()||"";if(""===n){const t=`FATAL: Required config element #${e} is empty. Processing stopped.`;throw console.error(t),new Error(t)}return s(`Required config read from #${e}: "${n}"`),n}(vt),t={statusPanelContainer:yt(gt,ht),titleSelector:yt(ft,pt),instructorHash:yt(bt,mt),dbName:e};return s("Configuration loaded:",t),t}async function xt(e){const t=(new TextEncoder).encode(e),n=await crypto.subtle.digest("SHA-256",t);return Array.from(new Uint8Array(n)).map(e=>e.toString(16).padStart(2,"0")).join("")}function St(e){return`${l.PIN_ATTEMPTS}:${e}`}function Et(e){const t=St(e),n=sessionStorage.getItem(t);if(!n)return null;try{return JSON.parse(n)}catch{return null}}function Ct(e){const t=Et(e);if(!t||!t.lockoutUntil)return{isLocked:!1,remainingMs:0};const n=new Date(t.lockoutUntil).getTime(),s=Date.now();return n>s?{isLocked:!0,remainingMs:n-s}:($t(e),{isLocked:!1,remainingMs:0})}function $t(e){const n=Et(e);n&&n.attempts>0&&s(`Cleared ${n.attempts} failed PIN attempts for ${t(e)} on successful login`);const o=St(e);sessionStorage.removeItem(o)}var It=Object.getOwnPropertyDescriptor;let At=class extends i{render(){return Ke`
  ...
  `,Tt([lt({type:String})],_t.prototype,"title",2),Tt([ut()],_t.prototype,"name",2),Tt([ut()],_t.prototype,"serviceId",2),Tt([ut()],_t.prototype,"instructorPassword",2),Tt([ut()],_t.prototype,"showInstructorModal",2),Tt([ut()],_t.prototype,"errorMessage",2),Tt([ut()],_t.prototype,"isSubmitting",2),Tt([ut()],_t.prototype,"pin",2),Tt([ut()],_t.prototype,"lockoutSeconds",2),_t=Tt([at("qd-login")],_t);var Lt=Object.defineProperty,Nt=Object.getOwnPropertyDescriptor,Pt=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?Nt(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&Lt(t,n,r),r};let zt=class extends i{constructor(){super(...arguments),this.total=0,this.correct=0,this.percentage=0,this.statusColor="red",this.name="",this.serviceId="",this.handleStateChanged=()=>{this.loadCache()},this.handleLogin=()=>{this.updateVisibility(),this.loadCache()},this.handleLogoutEvent=()=>{this.updateVisibility()}}connectedCallback(){super.connectedCallback(),this.updateVisibility(),this.loadCache(),document.addEventListener("qd:state-changed",this.handleStateChanged),document.addEventListener("qd:login",this.handleLogin),document.addEventListener("qd:logout",this.handleLogoutEvent)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("qd:state-changed",this.handleStateChanged),document.removeEventListener("qd:login",this.handleLogin),document.removeEventListener("qd:logout",this.handleLogoutEvent)}render(){const e=this.serviceId.slice(-4);return Ke`
  ...
  `,Pt([ut()],zt.prototype,"total",2),Pt([ut()],zt.prototype,"correct",2),Pt([ut()],zt.prototype,"percentage",2),Pt([ut()],zt.prototype,"statusColor",2),Pt([ut()],zt.prototype,"name",2),Pt([ut()],zt.prototype,"serviceId",2),zt=Pt([at("qd-status")],zt);const Ot=pe`
  :host {
    display: inline-block;
    font-family:
      system-ui,
...
`;class RateLimiter{constructor(){this.failureCount=0,this.lockoutUntil=null}attempt(){return!(this.lockoutUntil&&Date.now()<this.lockoutUntil)&&(this.lockoutUntil&&Date.now()>=this.lockoutUntil&&(this.lockoutUntil=null),!0)}recordFailure(){this.failureCount++;const e=[2e3,4e3,8e3,16e3,3e4],t=e[Math.min(this.failureCount-1,e.length-1)]??3e4;this.lockoutUntil=Date.now()+t}reset(){this.failureCount=0,this.lockoutUntil=null}getRemainingSeconds(){if(!this.lockoutUntil)return 0;const e=Math.max(0,this.lockoutUntil-Date.now());return Math.ceil(e/1e3)}isLockedOut(){return null!==this.lockoutUntil&&Date.now()<this.lockoutUntil}}const Dt="instructor.password.hash";var Mt=Object.defineProperty,Rt=Object.getOwnPropertyDescriptor,Ht=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?Rt(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&Mt(t,n,r),r};let Ut=class extends i{constructor(){super(...arguments),this.password="",this.error="",this.remainingSeconds=0,this.rateLimiter=new RateLimiter,this.handlePasswordInput=e=>{const t=e.target;this.password=t.value,this.error=""},this.handleSubmit=async e=>{e.preventDefault();if(!this.rateLimiter.attempt())return this.remainingSeconds=this.rateLimiter.getRemainingSeconds(),this.startCountdown(),void(this.error=`Too many attempts. Try again in ${this.remainingSeconds}s`);try{const e=function(){const e=document.getElementById(Dt);if(!e){const e=`Instructor password hash not found. Expected element with id="${Dt}". Check Oxygen XSL transform configuration.`;throw o(e),new Error(e)}const t=e.textContent?.trim();if(!t){const e="Instructor password hash element is empty. Check Oxygen parameter configuration.";throw o(e),new Error(e)}if(!/^[a-f0-9]{64}$/i.test(t)){const e=`Invalid password hash format. Expected 64 hex characters (SHA-256), got: ${t.substring(0,20)}...`;throw o(e),new Error(e)}return t.toLowerCase()}(),t=(new TextEncoder).encode(this.password),n=await crypto.subtle.digest("SHA-256",t),s=Array.from(new Uint8Array(n)).map(e=>e.toString(16).padStart(2,"0")).join(""),r=await async function(e,t){if(e.length!==t.length)return!1;if(0===e.length)return!0;const n=new TextEncoder,s=n.encode(e),o=n.encode(t);try{const e=await crypto.subtle.importKey("raw",s,{name:"HMAC",hash:"SHA-256"},!1,["sign"]),t=await crypto.subtle.sign("HMAC",e,o),n=await crypto.subtle.importKey("raw",o,{name:"HMAC",hash:"SHA-256"},!1,["sign"]),r=await crypto.subtle.sign("HMAC",n,s);if(t.byteLength!==r.byteLength)return!1;const a=new Uint8Array(t),d=new Uint8Array(r);let c=0;for(let s=0;s<a.length;s++)c|=(a[s]??0)^(d[s]??0);return 0===c}catch(r){return console.error("Constant-time comparison failed:",r),!1}}(s,e);r?(this.rateLimiter.reset(),this.password="",this.error="",x(this,"qd:instructor-unlock",{})):(this.error="Invalid password",this.password="")}catch{this.error="Authentication failed",this.password=""}}}disconnectedCallback(){super.disconnectedCallback(),this.countdownInterval&&window.clearInterval(this.countdownInterval)}startCountdown(){this.countdownInterval&&window.clearInterval(this.countdownInterval),this.countdownInterval=window.setInterval(()=>{this.remainingSeconds=this.rateLimiter.getRemainingSeconds(),0===this.remainingSeconds?(this.countdownInterval&&(window.clearInterval(this.countdownInterval),this.countdownInterval=void 0),this.error=""):this.error=`Too many attempts. Try again in ${this.remainingSeconds}s`},1e3)}render(){const e=this.remainingSeconds>0;return Ke`
      <div class="unlock-container">
        ...
        <form @submit=${this.handleSubmit}>
          ...
          ${this.error?Ke`<div class="error" role="alert" aria-live="polite">${this.error}</div>`:""}
    ...
    `}};Kt.styles=Ot,Jt([lt({type:Array})],Kt.prototype,"students",2),Kt=Jt([at("qd-instructor-export")],Kt);var Gt=Object.defineProperty,Yt=Object.getOwnPropertyDescriptor,Zt=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?Yt(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&Gt(t,n,r),r};let Xt=class extends i{constructor(){super(...arguments),this.showConfirmDialog=!1,this.confirmText="",this.error="",this.success="",this.modalContainer=null,this.handleClearRequest=()=>{this.showConfirmDialog=!0,this.confirmText="",this.error="",this.success=""},this.handleCancelClear=()=>{this.showConfirmDialog=!1,this.confirmText="",this.error=""},this.handleConfirmInput=e=>{const t=e.target;this.confirmText=t.value},this.handleConfirmClear=()=>{if("DELETE ALL DATA"===this.confirmText)try{C(),x(this,"qd:data-cleared",{}),this.success="All quiz data cleared successfully",this.showConfirmDialog=!1,this.confirmText="",this.error="",setTimeout(()=>{this.success=""},3e3)}catch{this.error="Failed to clear data"}else this.error="Confirmation text does not match"}}disconnectedCallback(){super.disconnectedCallback(),this.removeModalFromBody()}updated(e){super.updated(e),e.has("showConfirmDialog")&&(this.showConfirmDialog?this.renderModalToBody():this.removeModalFromBody()),this.showConfirmDialog&&(e.has("confirmText")||e.has("error"))&&this.renderModalToBody()}renderModalToBody(){this.modalContainer||(this.modalContainer=document.createElement("div"),this.modalContainer.className="qd-manage-modal-container",document.body.appendChild(this.modalContainer)),ot(this.renderConfirmDialog(),this.modalContainer)}removeModalFromBody(){this.modalContainer&&(this.modalContainer.remove(),this.modalContainer=null)}render(){return Ke`
      <button
      ...
      ${this.success?Ke`
          ...
          `:""}
    `}renderConfirmDialog(){const e="DELETE ALL DATA"===this.confirmText;return Ke`
      <div
      ...
      >
        ...
        >
          ...
          ${this.error?Ke`<div style="color: #dc3545; font-size: 14px; margin: 8px 0;">${this.error}</div>`:""}
  ...
  `,nn([lt({type:Array})],sn.prototype,"students",2),nn([lt({type:Boolean})],sn.prototype,"showModal",2),nn([ut()],sn.prototype,"searchText",2),nn([ut()],sn.prototype,"confirmingStudent",2),sn=nn([at("qd-pin-reset-dialog")],sn);var on=Object.defineProperty,rn=Object.getOwnPropertyDescriptor,an=(e,t,n,s)=>{for(var o,r=s>1?void 0:s?rn(t,n):t,a=e.length-1;a>=0;a--)(o=e[a])&&(r=(s?o(t,n,r):o(r))||r);return s&&r&&on(t,n,r),r};let dn=class extends i{constructor(){super(...arguments),this.unlocked=!1,this.showScores=!1,this.students=[],this.showStudentAnswers=!1,this.showPinReset=!1,this.handleLoginEvent=e=>{const t=e,n=t.detail?.role;this.updateVisibility(),"instructor"===n&&this.unlock()},this.handleLogoutEvent=()=>{this.updateVisibility(),this.lock()},this.handleResetPins=async()=>{const e=S(l.SESSION);if(e){try{const{getStorageService:t}=await Promise.resolve().then(()=>F),n=t(),s=await n.getStudentsByRelease(e.release);this.students=s}catch(t){console.error("Failed to load students:",t),this.students=[]}this.showPinReset=!0}},this.handleClosePinReset=()=>{this.showPinReset=!1},this.handlePinReset=()=>{this.dispatchEvent(new CustomEvent("qd:pin-reset",{bubbles:!0,composed:!0}))},this.handleUnlock=()=>{this.unlocked=!0,this.dispatchEvent(new CustomEvent("qd:instructor-unlock",{bubbles:!0,composed:!0}))},this.handleViewScores=async()=>{const e=S(l.SESSION);if(e){try{const{getStorageService:t}=await Promise.resolve().then(()=>F),n=t(),s=await n.getStudentsByRelease(e.release);this.students=s}catch(t){console.error("Failed to load students:",t),this.students=[]}this.showScores=!0}},this.handleCloseScores=()=>{this.showScores=!1},this.handleDataCleared=()=>{this.dispatchEvent(new CustomEvent("qd:data-cleared",{bubbles:!0,composed:!0})),this.students=[]},this.handleLogout=()=>{const e=S(l.SESSION);(new SessionService).clearSession(),this.dispatchEvent(new CustomEvent("qd:logout",{detail:{serviceId:e?.serviceId||"unknown"},bubbles:!0,composed:!0}))},this.handleToggleStudentAnswers=async e=>{const t=e.target;if(this.showStudentAnswers=t.checked,this.showStudentAnswers&&0===this.students.length){const e=S(l.SESSION);if(e)try{const{getStorageService:t}=await Promise.resolve().then(()=>F),n=t(),s=await n.getStudentsByRelease(e.release);this.students=s}catch(s){console.error("Failed to load students for toggle:",s)}}const n=this.showStudentAnswers?"qd:instructor-show-answers":"qd:instructor-hide-answers";this.dispatchEvent(new CustomEvent(n,{bubbles:!0,composed:!0})),sessionStorage.setItem("qd/instructor/showAnswers",String(this.showStudentAnswers))}}connectedCallback(){super.connectedCallback(),this.updateVisibility();const e="true"===sessionStorage.getItem(l.INSTRUCTOR);e&&this.unlock();const t=sessionStorage.getItem("qd/instructor/showAnswers");null!==t&&(this.showStudentAnswers="true"===t,this.showStudentAnswers&&e&&setTimeout(()=>{this.dispatchEvent(new CustomEvent("qd:instructor-show-answers",{bubbles:!0,composed:!0}))},100)),document.addEventListener("qd:login",this.handleLoginEvent),document.addEventListener("qd:logout",this.handleLogoutEvent)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("qd:login",this.handleLoginEvent),document.removeEventListener("qd:logout",this.handleLogoutEvent)}updateVisibility(){"true"===sessionStorage.getItem(l.INSTRUCTOR)?this.setAttribute("data-show",""):this.removeAttribute("data-show")}setStudents(e){this.students=e}unlock(){this.unlocked=!0}lock(){this.unlocked=!1,this.showScores=!1,this.showPinReset=!1}render(){return this.unlocked?Ke`
    ...
    `],an([ut()],dn.prototype,"unlocked",2),an([ut()],dn.prototype,"showScores",2),an([ut()],dn.prototype,"students",2),an([ut()],dn.prototype,"showStudentAnswers",2),an([ut()],dn.prototype,"showPinReset",2),dn=an([at("qd-instructor")],dn);const cn={statusPanel:".wh_top_menu_and_indexterms_link"};function ln(e={}){const t=e.statusPanelContainer||cn.statusPanel;!function(e){const t=document.querySelector(e);if(!t)return s(`Login component not injected: container '${e}' not found`),null;const n=document.createElement("qd-login");t.appendChild(n),s("Login component injected")}(t),function(e){const t=document.querySelector(e);if(!t)return s(`Status component not injected: container '${e}' not found`),null;const n=document.createElement("qd-status");t.appendChild(n),s("Status component injected")}(t),function(e){const t=document.querySelector(e);if(!t)return s(`Instructor component not injected: container '${e}' not found`),null;const n=document.createElement("qd-instructor");t.appendChild(n),s("Instructor component injected")}(t)}const un={red:"qd-badge-red",amber:"qd-badge-amber",green:"qd-badge-green"},hn={unstarted:"red",incomplete:"amber",complete:"green"};function pn(e){const t=function(e,t){if(!e||!t?.pages)return"unstarted";const n=t.pages[e];return n?.state??"unstarted"}(e.getAttribute("data-page-id"),S(l.CACHE));!function(e,t){Object.values(un).forEach(t=>{e.classList.remove(t)});const n=un[hn[t]];e.classList.add(n)}(e,t)}function mn(){const e=document.querySelectorAll(".quizPageBtn"),t=S(l.CACHE),n="true"===sessionStorage.getItem(l.INSTRUCTOR);if(!t||n)return e.forEach(e=>{Object.values(un).forEach(t=>{e.classList.remove(t)})}),void s(n?`Removed badge styling from ${e.length} page links (instructor mode)`:`Removed badge styling from ${e.length} page links (no session)`);e.forEach(e=>{pn(e)}),s(`Updated ${e.length} page badges`)}function gn(e){const t=e,{pageId:n}=t.detail,o=document.querySelector(`[data-page-id="${n}"]`);o&&o.classList.contains("quizPageBtn")&&(pn(o),s(`Updated badge for page ${n}`))}function fn(){s("Cache rebuilt, refreshing all badges"),mn()}function bn(){s("Logout detected, removing all badge styling");const e=document.querySelectorAll(".quizPageBtn");e.forEach(e=>{Object.values(un).forEach(t=>{e.classList.remove(t)})}),s(`Removed badge styling from ${e.length} page links`)}const vn={initialized:!1};async function yn(e={}){if(vn.initialized)return void r("Bootstrap already initialized, skipping");if(s("Bootstrapping Sonar Quiz System..."),function(){if(document.getElementById("qd-global-styles"))return;const e=document.createElement("style");e.id="qd-global-styles",e.textContent="\n    /* Sonar Quiz System - Global Styles */\n    .qd-hidden {\n      display: none !important;\n    }\n\n    /* Quiz table interactive mode styles */\n    .qd-quiz-interactive .qd-quiz-input {\n      width: 100%;\n      padding: 0.5rem;\n      font-size: 1rem;\n      border: 1px solid #ccc;\n      border-radius: 4px;\n    }\n\n    /* Validation styling for answer cells */\n    .qd-quiz-interactive .qd-answer-correct {\n      background-color: #d4edda !important;\n      border-color: #28a745 !important;\n    }\n\n    .qd-quiz-interactive .qd-answer-incorrect {\n      background-color: #f8d7da !important;\n      border-color: #dc3545 !important;\n    }\n\n    /* Home page badge styles (R/A/G indicators) */\n    .qd-badge-red {\n      border-left: 4px solid #d32f2f !important;\n      background-color: #ffebee !important;\n    }\n\n    .qd-badge-amber {\n      border-left: 4px solid #ff9800 !important;\n      background-color: #fff3e0 !important;\n    }\n\n    .qd-badge-green {\n      border-left: 4px solid #4caf50 !important;\n      background-color: #e8f5e9 !important;\n    }\n\n    /* Instructor mode: Student answers display */\n    .qd-student-answers {\n      margin-top: 12px;\n      padding: 8px;\n      background: #f8f9fa;\n      border-radius: 4px;\n      border: 1px solid #dee2e6;\n    }\n\n    .qd-student-answer {\n      font-size: 12px;\n      padding: 4px 0;\n      line-height: 1.4;\n    }\n\n    .qd-student-answer.qd-correct {\n      color: #28a745;\n    }\n\n    .qd-student-answer.qd-incorrect {\n      color: #dc3545;\n    }\n\n    .qd-student-name {\n      font-weight: 600;\n    }\n\n    .qd-student-answer-text {\n      margin: 0 4px;\n    }\n\n    .qd-timestamp {\n      color: #6c757d;\n      font-size: 11px;\n      margin-left: 8px;\n    }\n  ",document.head.appendChild(e),s("Global styles injected")}(),!e.dbName){const e="FATAL: dbName not provided in bootstrap config. Processing stopped.";throw console.error(e),new Error(e)}const t=B(e.dbName);await t.init();const n=new EventCoordinator;n.initialize(),vn.eventCoordinator=n;const o=new SessionCoordinator;o.initialize(),vn.sessionCoordinator=o,ln({statusPanelContainer:e.statusPanelContainer,dbName:e.dbName}),!1!==e.autoEnhanceQuizTables&&function(){const e=document.querySelectorAll("table.qd-quiz");if(0===e.length)return void s("No quiz tables found to enhance");s(`Enhancing ${e.length} quiz table(s) in non-interactive mode...`);let t=0;for(const s of Array.from(e))try{W(s,{interactive:!1}),t++}catch(n){r(`Failed to enhance quiz table: ${n.message}`)}s(`Enhanced ${t} of ${e.length} quiz table(s) (non-interactive)`)}(),!1!==e.autoEnhanceAnalysisTables&&function(){const e=document.querySelectorAll("table.qd-analysis");if(0===e.length)return void s("No analysis tables found to enhance");s(`Enhancing ${e.length} analysis table(s) in non-interactive mode...`);let t=0;for(const s of Array.from(e))try{ie(s,{interactive:!1}),t++}catch(n){r(`Failed to enhance analysis table: ${n.message}`)}s(`Enhanced ${t} of ${e.length} analysis table(s) (non-interactive)`)}(),!1!==e.autoEnhanceHomeBadges&&function(){const e=document.querySelectorAll(".quizPageBtn");if(0===e.length)return void s("No .quizPageBtn links found, skipping badge enhancement");s(`Enhancing home page badges for ${e.length} link(s)...`);try{document.querySelectorAll(".quizPageBtn").forEach(e=>{const t=function(e){const t=e.getAttribute("href");return t&&t.substring(t.lastIndexOf("/")+1).replace(/\.html?$/i,"")||null}(e);t?(e.setAttribute("data-page-id",t),s(`Set data-page-id="${t}" for link: ${e.textContent?.trim()}`)):s(`Failed to extract pageId from href: ${e.getAttribute("href")}`)}),mn(),document.addEventListener("qd:state-changed",gn),document.addEventListener("qd:cache-rebuild",fn),document.addEventListener("qd:logout",bn),s("Home page badges enhanced with event listeners"),s("Home page badges enhanced")}catch(t){r(`Failed to enhance home badges: ${t.message}`)}}(),await async function(){const e=S(l.SESSION);if(!e)return void s("No existing session, tables remain in non-interactive mode");if("true"===sessionStorage.getItem(l.INSTRUCTOR)){s("Instructor session detected, revealing answers in non-interactive tables");const e=window.location.pathname,t=e.substring(e.lastIndexOf("/")+1).replace(/\.html?$/i,"");return void document.querySelectorAll("table.qd-quiz").forEach(e=>{const n=Y(e);if(!n)return;n.pageId=t;e.querySelectorAll("td:nth-child(2), th:nth-child(2)").forEach(e=>{e.classList.remove("qd-hidden")});e.querySelectorAll("tbody td:nth-child(2)").forEach((e,t)=>{const s=n.parsed.questions[t];s&&e instanceof HTMLTableCellElement&&(e.textContent=s.correctAnswer)});e.querySelectorAll("td:nth-child(3), th:nth-child(3)").forEach(e=>e.classList.remove("qd-hidden"));const s=()=>{Z(e,n)},o=()=>{X(e)};document.addEventListener("qd:instructor-show-answers",s),document.addEventListener("qd:instructor-hide-answers",o);"true"===sessionStorage.getItem("qd/instructor/showAnswers")&&s()})}s(`Existing session detected for ${e.serviceId}, upgrading tables to interactive mode`);const t=B();let n=S(l.CACHE);if(!n){s("Cache not found, rebuilding from IndexedDB...");try{const o=await t.loadStudentRecord(e);n=t.buildCache(o),E(l.CACHE,n),s(`Cache rebuilt from IndexedDB: ${n.totals.total} total questions`)}catch{r("Failed to rebuild cache from IndexedDB, using empty cache"),n={totals:{total:0,answered:0,correct:0},pages:{}},E(l.CACHE,n)}}const o=window.location.pathname,a=o.substring(o.lastIndexOf("/")+1).replace(/\.html?$/i,"");if(!a)return void s("No pageId found, skipping table upgrade");const d=document.querySelectorAll("table.qd-quiz");d.length>0&&(s(`Upgrading ${d.length} quiz table(s) to interactive mode...`),d.forEach(e=>{W(e,{interactive:!0,pageId:a})}));const c=document.querySelectorAll("table.qd-analysis");c.length>0&&(s(`Upgrading ${c.length} analysis table(s) to interactive mode...`),c.forEach(e=>{ie(e,{interactive:!0,pageId:a})}))}(),vn.initialized=!0,s("Bootstrap complete")}if("undefined"!=typeof window){const e=()=>{s("Auto-initializing Sonar Quiz System");const e=wt();yn({dbName:e.dbName,statusPanelContainer:e.statusPanelContainer,autoEnhanceQuizTables:!0,autoEnhanceAnalysisTables:!0,autoEnhanceHomeBadges:!0}).catch(e=>{console.error("[FATAL] Bootstrap failed:",e)})};"loading"===document.readyState?document.addEventListener("DOMContentLoaded",()=>{e()}):e()}return e.BUILD_DATE="25/Nov/2025",e.DEFAULT_CONTAINERS=cn,e.Debouncer=Debouncer,e.SCHEMA_VERSION=2,e.SESSION_TIMEOUT_MS=c,e.STORAGE_KEYS=l,e.VERSION="0.1.0-phase3.1",e.bootstrap=yn,e.calculateCompletionState=D,e.cleanup=function(){vn.initialized?(s("Cleaning up bootstrap resources..."),vn.eventCoordinator?.cleanup(),vn.sessionCoordinator?.cleanup(),vn.initialized=!1,vn.eventCoordinator=void 0,vn.sessionCoordinator=void 0,s("Bootstrap cleanup complete")):r("Bootstrap not initialized, nothing to cleanup")},e.clearQuizData=C,e.enhanceAnalysisTable=ie,e.enhanceQuizTable=W,e.error=o,e.generateCellKey=ne,e.generateTableId=te,e.getAnalysisTableMetadata=function(e){return re.get(e)},e.getJSON=S,e.getQuizTableMetadata=Y,e.info=s,e.injectComponents=ln,e.isAnalysisTableEnhanced=function(e){return re.has(e)},e.isCellEditable=se,e.isInitialized=function(){return vn.initialized},e.isQuizTableEnhanced=function(e){return Q.has(e)},e.parseAnalysisTable=oe,e.parseQuizTable=a,e.setJSON=E,e.validateAnswer=d,e.warn=r,Object.defineProperty(e,Symbol.toStringTag,{value:"Module"}),e}({});
